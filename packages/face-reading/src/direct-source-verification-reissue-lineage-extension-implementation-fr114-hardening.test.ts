...
describe('FR114 direct-source verification lineage implementation hardening', () => {
  it('rejects pretending the implementation itself authorizes or persists the target reissue', () => {
    const artifact = validArtifact();
    const promoted = {
      ...artifact,
      implementation: {
        ...artifact.implementation,
        targetRelationPersisted: true,
        targetReissueRecordPersisted: true,
      },
      execution: {
        ...artifact.execution,
    ...
    } as unknown as DirectSourceVerificationReissueLineageExtensionImplementationFR114V1;
    expect(() => validateDirectSourceVerificationReissueLineageExtensionImplementationFR114(promoted))
      .toThrow(FaceAuthorityValidationError);
  });

  it('rejects loss of non-independent and exact-evidence invariants', () => {
    const artifact = validArtifact();
    const weakened = {
      ...artifact,
      implementation: {
        ...artifact.implementation,
        exactEvidenceReuseEnforced: false,
        sameCheckingEventEnforced: false,
        independentVerificationDeltaFixedAtZero: false,
        childMayCountAsIndependentVerification: true,
      },
    } as unknown as DirectSourceVerificationReissueLineageExtensionImplementationFR114V1;
    expect(() => validateDirectSourceVerificationReissueLineageExtensionImplementationFR114(weakened))
      .toThrow(FaceAuthorityValidationError);
  });
  ...
  it('rejects shortcut or frontier drift', () => {
    const artifact = validArtifact();
    const drifted = {
      ...artifact,
      authorityBoundary: {
        ...artifact.authorityBoundary,
        coreExtensionImplementationMeansReissueAuthority: true,
      },
      recommendedNextFrontier: 'persist_reissue_now',
    } as unknown as DirectSourceVerificationReissueLineageExtensionImplementationFR114V1;
    expect(() => validateDirectSourceVerificationReissueLineageExtensionImplementationFR114(drifted))
      .toThrow(FaceAuthorityValidationError);
  });
});
