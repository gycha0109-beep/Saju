...
async function artifact108() {
  const fr66 = await runPhotoToLipsContourNeutralSurfaceFR66({
    ...
    providerRunRef: 'fr108:registry-admission',
  ...
  return reviewFiveOfficerIntakeCriterionMethodologyRegistryAdmissionFR108(fr107);
}

describe('FR108 Five Officers intake criterion methodology registry admission review', () => {
  it('distinguishes structural research append validity from provenance-preserving admission', async () => {
    const result = await artifact108();
    expect(result.admissionReview.structuralResearchRegistryAppendValid).toBe(true);
    expect(result.admissionReview.candidateSourceRefResolves).toBe(true);
    expect(result.admissionReview.candidateSourceRefResolvesToHistoricalCtextPassage).toBe(true);
    expect(result.admissionReview.governedScanCheckedAuthorityPresentUpstream).toBe(true);
    expect(result.admissionReview.governedScanCheckedAuthorityPersistedInFaceRegistry).toBe(false);
    expect(result.admissionReview.scanCheckedWitnessAuthoritySurvivesCandidateSourceRefResolution).toBe(false);
    expect(result.admissionReview.provenancePreservingRegistryAdmissionValid).toBe(false);
    expect(result.admissionReview.registryAdmissionAuthorized).toBe(false);
  });

  it('records that the verified NLC witness exists but the registered intake passage still points to ctext/unverified OCR', async () => {
    const result = await artifact108();
    expect(result.faceRegistrySnapshot.nlcWitnessRegistered).toBe(true);
    expect(result.faceRegistrySnapshot.nlcWitnessStatus).toBe('verified');
    expect(result.faceRegistrySnapshot.intakePassageWitnessId).toBe('witness.shenxiang_quanbian.ctext');
    expect(result.faceRegistrySnapshot.intakePassageVerificationStatus).toBe('unverified_ocr');
    expect(result.faceRegistrySnapshot.intakePassageUsesNlcWitness).toBe(false);
    expect(result.faceRegistrySnapshot.sourceAuthorityOverlayCollectionSupportedByContract).toBe(false);
  });

  it('does not choose replacement, duplicate-id append, versioning, or sourceRef rewrite without a persistence design review', async () => {
    const result = await artifact108();
    expect(result.passageIdentityConstraint.passageIdsUniqueWithinRegistry).toBe(true);
    expect(result.passageIdentityConstraint.samePassageIdNlcAppendWithoutReplacingHistoricalEntryAuthorized).toBe(false);
    expect(result.passageIdentityConstraint.replaceHistoricalCtextPassageWithNlcAuthorized).toBe(false);
    expect(result.passageIdentityConstraint.introduceVersionedNlcPassageIdAuthorized).toBe(false);
    expect(result.passageIdentityConstraint.candidateSourceRefRewriteAuthorized).toBe(false);
    expect(result.passageIdentityConstraint.explicitPersistenceDesignReviewRequired).toBe(true);
  });

  it('persists nothing and issues no execution, metric, threshold, criterion, claim, or traditional authority', async () => {
    const result = await artifact108();
    expect(result.admissionReview.candidatePersisted).toBe(false);
    expect(result.admissionReview.methodologyDefinitionsPersisted).toBe(0);
    ...
    expect(result.recommendedNextFrontier).toBe('intake_scan_checked_passage_registry_identity_persistence_review');
    expect(validateFiveOfficerIntakeCriterionMethodologyRegistryAdmissionReviewFR108(result)).toBe(result);
  });
});
