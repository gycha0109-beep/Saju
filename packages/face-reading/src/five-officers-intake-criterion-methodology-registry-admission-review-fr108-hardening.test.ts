...
async function artifact108() {
  const fr66 = await runPhotoToLipsContourNeutralSurfaceFR66({
    ...
    providerRunRef: 'fr108:hardening',
...
function mutableCopy(source: FiveOfficerIntakeCriterionMethodologyRegistryAdmissionReviewFR108V1): Record<string, any> {
  return structuredClone(source) as unknown as Record<string, any>;
}

describe('FR108 methodology registry admission hardening', () => {
  it('rejects provenance loss being relabeled as registry admission', async () => {
    const baseline = await artifact108();
    const admitted = mutableCopy(baseline);
    admitted.admissionReview.provenancePreservingRegistryAdmissionValid = true;
    admitted.admissionReview.registryAdmissionAuthorized = true;
    admitted.admissionReview.candidatePersisted = true;
    admitted.admissionReview.methodologyDefinitionsPersisted = 1;
    expect(() => validateFiveOfficerIntakeCriterionMethodologyRegistryAdmissionReviewFR108(
      admitted as unknown as FiveOfficerIntakeCriterionMethodologyRegistryAdmissionReviewFR108V1,
    )).toThrow(FaceAuthorityValidationError);
  });

  it('rejects pretending verified witness presence is equivalent to persisted scan-checked passage authority', async () => {
    const baseline = await artifact108();
    const rewritten = mutableCopy(baseline);
    rewritten.faceRegistrySnapshot.intakePassageUsesNlcWitness = true;
    rewritten.faceRegistrySnapshot.intakePassageVerificationStatus = 'scan_checked';
    rewritten.admissionReview.governedScanCheckedAuthorityPersistedInFaceRegistry = true;
    rewritten.admissionReview.scanCheckedWitnessAuthoritySurvivesCandidateSourceRefResolution = true;
    expect(() => validateFiveOfficerIntakeCriterionMethodologyRegistryAdmissionReviewFR108(
      rewritten as unknown as FiveOfficerIntakeCriterionMethodologyRegistryAdmissionReviewFR108V1,
    )).toThrow(FaceAuthorityValidationError);
  });

  it('rejects unreviewed passage replacement/versioning and downstream semantic authority', async () => {
    const baseline = await artifact108();
    const shortcut = mutableCopy(baseline);
    shortcut.passageIdentityConstraint.replaceHistoricalCtextPassageWithNlcAuthorized = true;
    shortcut.passageIdentityConstraint.introduceVersionedNlcPassageIdAuthorized = true;
    shortcut.passageIdentityConstraint.candidateSourceRefRewriteAuthorized = true;
    shortcut.execution.metricBindingsIssued = 1;
    shortcut.execution.thresholdsIssued = 1;
    shortcut.execution.criterionStatesIssued = 1;
    shortcut.execution.traditionalSemanticAuthority = true;
    expect(() => validateFiveOfficerIntakeCriterionMethodologyRegistryAdmissionReviewFR108(
      shortcut as unknown as FiveOfficerIntakeCriterionMethodologyRegistryAdmissionReviewFR108V1,
    )).toThrow(FaceAuthorityValidationError);
  });
});
