...
export const FR269_CONTRACT_VERSION =
  'FR269-CONTROLLED-CAPTURE-GEOMETRY-SENSITIVITY-v1' as const;

export const FR269_NEXT_FRONTIER =
...
export type FR269ControlledCondition =
  | 'baseline_eye_level'
  | 'low_angle'
  | 'high_angle';

export interface FR269ScalarSummary {
...
export interface FR269ConditionSummary {
  readonly condition: FR269ControlledCondition;
  readonly sourceRef: string;
  readonly sourceGeneratedAt: string;
  readonly captureCount: 4;
  readonly primaryMetricDegrees: FR269ScalarSummary;
  readonly geometry: {
    ...
    readonly relativeRotationFromFirstAcceptedCaptureRadians: FR269ScalarSummary;
    readonly relativeRotationFromFirstAcceptedCaptureDegrees: FR269ScalarSummary;
...
export interface FR269BaselineRelativeContrast {
...
export interface FR269ControlledGeometrySensitivityReport {
  readonly schemaVersion: 'fr269-controlled-capture-geometry-sensitivity-report-v1';
  readonly artifactVersion: '0.1.0';
  readonly contractVersion: typeof FR269_CONTRACT_VERSION;
  ...
  readonly conditions: readonly [
    FR269ConditionSummary,
    FR269ConditionSummary,
    FR269ConditionSummary,
  ];
  readonly baselineRelativeContrasts: readonly [
    FR269BaselineRelativeContrast,
    FR269BaselineRelativeContrast,
  ];
  ...
  readonly nextFrontier: typeof FR269_NEXT_FRONTIER;
}

export interface FR269SameFrameEyeTiltDiagnosticEvidence {
  readonly schemaVersion: 'fr269-same-frame-eye-tilt-diagnostic-evidence-v1';
  readonly artifactVersion: '0.1.0';
  readonly contractVersion: typeof FR269_CONTRACT_VERSION;
  readonly authorityState:
...
export interface FR269SameFrameEyeTiltDiagnosticSlot {
  readonly schemaVersion: 'fr269-same-frame-eye-tilt-diagnostic-slot-v1';
...
export interface FR269SameFrameEyeTiltDiagnosticBundle {
  readonly schemaVersion: 'fr269-same-frame-eye-tilt-diagnostic-bundle-v1';
  readonly artifactVersion: '0.1.0';
  readonly contractVersion: typeof FR269_CONTRACT_VERSION;
  readonly authorityState:
    'same_frame_eye_tilt_diagnostic_scalar_only_no_metric_replacement';
  readonly generatedAt: string;
  readonly slots: readonly FR269SameFrameEyeTiltDiagnosticSlot[];
  readonly descriptiveSummary: {
    readonly acceptedDiagnosticCount: number;
    readonly screenSpaceEyeOuterCornerTiltMeanDegrees: FR269ScalarSummary | null;
    readonly fr76CanonicalMetricEyeOuterCornerTiltMeanDegrees: FR269ScalarSummary | null;
    readonly screenMinusFr76Degrees: FR269ScalarSummary | null;
  };
  readonly persistenceBoundary: {
...
function degreesSummary(values: readonly number[]): FR269ScalarSummary {
...
function contrast(
  baseline: FR269ConditionSummary,
  condition: FR269ConditionSummary,
): FR269BaselineRelativeContrast {
...
export function analyzeControlledCaptureGeometrySensitivityFR269(input: {
  readonly generatedAt: string;
  readonly conditions: readonly {
    readonly condition: FR269ControlledCondition;
    readonly sourceRef: string;
...
}): FR269ControlledGeometrySensitivityReport {
  ...
  return Object.freeze({
    schemaVersion: 'fr269-controlled-capture-geometry-sensitivity-report-v1' as const,
    artifactVersion: '0.1.0' as const,
    contractVersion: FR269_CONTRACT_VERSION,
    authorityState:
    ...
    nextFrontier: FR269_NEXT_FRONTIER,
  });
}
...
export function deriveSameFrameEyeTiltDiagnosticFR269(
  observation: FR257EphemeralGeometryObservation,
): FR269SameFrameEyeTiltDiagnosticEvidence {
  ...
  return Object.freeze({
    schemaVersion: 'fr269-same-frame-eye-tilt-diagnostic-evidence-v1' as const,
    artifactVersion: '0.1.0' as const,
    contractVersion: FR269_CONTRACT_VERSION,
    authorityState:
...
export function createSameFrameEyeTiltDiagnosticCollectorFR269():
FR269SameFrameEyeTiltDiagnosticCollector {
  const pending = new Map<string, FR269SameFrameEyeTiltDiagnosticEvidence>();
  const observe = (observation: FR257EphemeralGeometryObservation): void => {
    ...
    pending.set(
      ...
      deriveSameFrameEyeTiltDiagnosticFR269(observation),
    );
  };
  return Object.freeze({
    observe,
...
export function bindSameFrameEyeTiltDiagnosticSlotFR269(input: {
  readonly record: FR243DryRunCaptureExecutionRecord;
  readonly providerRunRef: string;
  readonly evidence: FR269SameFrameEyeTiltDiagnosticEvidence | null;
  ...
  if (input.record.resultStatus === 'accepted_for_dry_run_mechanics_only') {
    if (
      ...
      fail('accepted FR243 capture requires exact FR269 same-frame evidence.');
    }
    return Object.freeze({
  ...
  if (input.evidence !== null || input.record.primaryMetric !== null) {
    fail('rejected capture must not retain FR269 scalar diagnostic evidence.');
  }
  return Object.freeze({
    schemaVersion: 'fr269-same-frame-eye-tilt-diagnostic-slot-v1' as const,
    sessionOrdinal: input.record.sessionOrdinal,
...
export function buildSameFrameEyeTiltDiagnosticBundleFR269(input: {
  readonly generatedAt: string;
  readonly slots: readonly FR269SameFrameEyeTiltDiagnosticSlot[];
}): FR269SameFrameEyeTiltDiagnosticBundle {
  exactIso(input.generatedAt);
  if (input.slots.length !== 4) fail('FR269 diagnostic bundle requires four slots.');
  const expected = ['1:1', '1:2', '2:1', '2:2'];
  input.slots.forEach((slot, index) => {
    if (`${slot.sessionOrdinal}:${slot.captureOrdinal}` !== expected[index]) {
      fail('FR269 diagnostic slots must remain ordered 1:1,1:2,2:1,2:2.');
    }
  });
