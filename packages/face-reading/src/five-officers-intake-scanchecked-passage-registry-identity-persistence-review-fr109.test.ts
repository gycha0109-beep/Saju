...
async function artifact109() {
  const fr66 = await runPhotoToLipsContourNeutralSurfaceFR66({
    ...
    providerRunRef: 'fr109:identity-persistence',
  ...
  return reviewFiveOfficerIntakeScanCheckedPassageRegistryIdentityPersistenceFR109(fr108);
}

describe('FR109 intake scan-checked passage registry identity persistence review', () => {
  it('distinguishes structural validity from authority admission across persistence strategies', async () => {
    const result = await artifact109();
    expect(result.strategyReviews.map((entry) => [entry.strategy, entry.structurallyValidUnderCurrentFaceRegistry])).toEqual([
      ['same_id_duplicate_append', false],
      ['same_id_historical_replacement', true],
      ['witness_qualified_new_passage_identity', true],
      ['registry_overlay_schema_extension', false],
    ]);
    expect(result.strategyReviews.every((entry) => entry.authorityAdmitted === false)).toBe(true);
  });

  it('prefers a witness-qualified identity candidate while withholding persistence authority', async () => {
    const result = await artifact109();
    expect(result.decision.preferredStrategyCandidate).toBe('witness_qualified_new_passage_identity');
    expect(result.decision.proposedPassageRef).toBe('passage.shenxiang.five_officers.intake.nlc_1925');
    expect(result.decision.proposedWitnessId).toBe('witness.shenxiang_quanbian.nlc_1925');
    expect(result.decision.proposedVerificationStatus).toBe('scan_checked');
    expect(result.decision.proposedPassageStructurallyValid).toBe(true);
    expect(result.decision.newPassageIdentityDefinitionAdmitted).toBe(false);
    expect(result.decision.newVerificationRecordAuthorized).toBe(false);
    expect(result.decision.methodologySourceRefRewriteAuthorized).toBe(false);
    expect(result.decision.persistentRegistryMutationAuthorized).toBe(false);
  });

  it('retains the historical passage and rejects implicit FR104 passage-id aliasing', async () => {
    const result = await artifact109();
    expect(result.directSourceIdentityContract.verificationRecordPassageId).toBe('passage.shenxiang.five_officers.intake');
    expect(result.directSourceIdentityContract.materializedPassagePreservesRecordPassageId).toBe(true);
    expect(result.directSourceIdentityContract.verificationRecordAllowsImplicitPassageAlias).toBe(false);
    expect(result.decision.historicalPassageRetained).toBe(true);
    expect(result.decision.sameIdReplacementRejectedAsAuthorityShortcut).toBe(true);
    expect(result.decision.implicitAliasFromFr104VerificationRejected).toBe(true);
  });

  it('issues no persistence, methodology execution, metric, threshold, criterion, claim, or traditional authority', async () => {
    const result = await artifact109();
    expect(result.execution.passagesPersisted).toBe(0);
    expect(result.execution.verificationRecordsReissued).toBe(0);
    ...
    expect(result.recommendedNextFrontier).toBe('intake_witness_qualified_passage_identity_definition_review');
    expect(validateFiveOfficerIntakeScanCheckedPassageRegistryIdentityPersistenceReviewFR109(result)).toBe(result);
  });
});
