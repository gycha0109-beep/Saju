...
import { reviewFiveOfficerIntakePageVerificationReissueLineageContractFR112 } from './five-officers-intake-page-verification-reissue-lineage-contract-review-fr112.js';
import {
...
async function artifact113() {
  ...
  return reviewDirectSourceVerificationReissueLineageExtensionAdmissionFR113(fr112);
}

describe('FR113 direct-source verification reissue lineage extension admission review', () => {
  it('admits only implementation of an optional additive registry-level relation collection', async () => {
    const result = await artifact113();
    expect(validateDirectSourceVerificationReissueLineageExtensionAdmissionReviewFR113(result)).toBe(result);
    expect(result.authorityState).toBe('additive_registry_relation_extension_admitted_for_implementation_core_unmodified');
    expect(result.proposedExtension).toMatchObject({
    ...
    expect(result.admissionReview).toMatchObject({
  ...
  it('keeps reissue, persistence, methodology, metric, threshold, criterion, claim, and semantic authority closed', async () => {
    const result = await artifact113();
    ...
    expect(result.recommendedNextFrontier).toBe('direct_source_verification_reissue_lineage_contract_extension_implementation');
  });
});
