...
async function artifact111() {
  const fr66 = await runPhotoToLipsContourNeutralSurfaceFR66({
    ...
    providerRunRef: 'fr111:reissue-admission',
...
describe('FR111 witness-qualified page-verification reissue admission review', () => {
  it('classifies the proposed record as an evidence-preserving identity derivative', async () => {
    const result = await artifact111();
    expect(result.originalVerification).toMatchObject({
    ...
    expect(result.proposedReissue).toMatchObject({
      ...
      differsFromOriginalOnlyByVerificationAndPassageIdentity: true,
      sameOriginalText: true,
      sameVisualEvidenceRefs: true,
      sameCheckerRefs: true,
      sameVerificationState: true,
      additionalVisualEvidenceRefsIssued: 0,
      additionalCheckerRefsIssued: 0,
      structuralRegistryValidationPassed: true,
    });
  });

  it('blocks reissue admission because the current verification contract cannot encode lineage', async () => {
    const result = await artifact111();
    expect(result.lineageContractReview).toEqual({
      directSourcePageVerificationRecordHasReissueLineageField: false,
      directSourceRegistryHasVerificationRelationCollection: false,
      proposedRecordCanNameOriginalVerificationAsParent: false,
      machineReadableReissueLineageAvailable: false,
      duplicateEvidenceCouldAppearAsIndependentVerificationRecord: true,
      independentVerificationCountMayIncreaseFromReissue: false,
      verificationReissueAdmissionAuthorized: false,
      verificationRecordPersistenceAuthorized: false,
      requiredContractAction: 'define_bounded_verification_reissue_lineage_before_admission',
    });
    expect(result.authorityState).toBe('witness_qualified_page_verification_reissue_blocked_missing_machine_readable_lineage_contract');
  });

  it('preserves FR104 and performs no registry or methodology mutation', async () => {
    const result = await artifact111();
    ...
    expect(result.execution.directSourceRegistrySchemaChanged).toBe(false);
    expect(result.execution.faceRegistryChanged).toBe(false);
  ...
  it('issues no metric, threshold, criterion, claim, or traditional semantic authority', async () => {
    const result = await artifact111();
    ...
    expect(result.recommendedNextFrontier).toBe('intake_page_verification_reissue_lineage_contract_review');
    expect(validateFiveOfficerIntakeWitnessQualifiedPageVerificationReissueAdmissionReviewFR111(result)).toBe(result);
  });
});
