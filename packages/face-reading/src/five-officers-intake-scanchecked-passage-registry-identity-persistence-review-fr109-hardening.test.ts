...
async function artifact109(): Promise<FiveOfficerIntakeScanCheckedPassageRegistryIdentityPersistenceReviewFR109V1> {
  const fr66 = await runPhotoToLipsContourNeutralSurfaceFR66({
    ...
    providerRunRef: 'fr109:hardening',
...
describe('FR109 intake passage identity persistence hardening', () => {
  it('rejects promotion of the preferred identity candidate to admitted authority', async () => {
    const result = await artifact109();
    const tampered = {
      ...result,
      decision: {
        ...result.decision,
        newPassageIdentityDefinitionAdmitted: true,
        persistentRegistryMutationAuthorized: true,
      },
      execution: {
        ...result.execution,
        passagesPersisted: 1,
      },
    } as unknown as FiveOfficerIntakeScanCheckedPassageRegistryIdentityPersistenceReviewFR109V1;
    expect(() => validateFiveOfficerIntakeScanCheckedPassageRegistryIdentityPersistenceReviewFR109(tampered))
      .toThrow(FaceAuthorityValidationError);
  });

  it('rejects rewriting the structural strategy matrix into authority equivalence', async () => {
    const result = await artifact109();
    const tampered = {
      ...result,
      strategyReviews: [
        result.strategyReviews[0],
        { ...result.strategyReviews[1], preservesHistoricalCtextPassageRecord: true, authorityAdmitted: true },
        result.strategyReviews[2],
        result.strategyReviews[3],
      ],
    } as unknown as FiveOfficerIntakeScanCheckedPassageRegistryIdentityPersistenceReviewFR109V1;
    expect(() => validateFiveOfficerIntakeScanCheckedPassageRegistryIdentityPersistenceReviewFR109(tampered))
      .toThrow(FaceAuthorityValidationError);
