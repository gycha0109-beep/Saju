...
describe('FR249 governed browser dry-run coordinator', () => {
  it('requires two explicit sessions x two explicit captures and returns the FR243 mechanics review', async () => {
    ...
    coordinator.beginSession({
      sessionOrdinal: 1,
      issuedAt: '2026-09-22T22:21:00.000Z',
    });
    ...
    for (const captureOrdinal of [1, 2] as const) {
      ...
      await coordinator.capture({
        challengeIssuedAt:
          captureOrdinal === 1
            ? '2026-09-22T22:21:10.000Z'
            : '2026-09-22T22:21:20.000Z',
        trigger: { timestampMs, providerRunRef },
        operatorExecutionAttestation: executionAttestation(
          captureOrdinal === 1
            ? '2026-09-22T22:21:11.000Z'
            : '2026-09-22T22:21:21.000Z',
        ),
        operatorQualityObservation: qualityObservation(providerRunRef, timestampMs),
      });
    }

    coordinator.beginSession({
      sessionOrdinal: 2,
      issuedAt: '2026-09-22T22:30:00.000Z',
    });
    ...
    for (const captureOrdinal of [1, 2] as const) {
      ...
      await coordinator.capture({
        challengeIssuedAt:
          captureOrdinal === 1
            ? '2026-09-22T22:30:10.000Z'
            : '2026-09-22T22:30:20.000Z',
        trigger: { timestampMs, providerRunRef },
        operatorExecutionAttestation: executionAttestation(
          captureOrdinal === 1
            ? '2026-09-22T22:30:11.000Z'
            : '2026-09-22T22:30:21.000Z',
        ),
        operatorQualityObservation: qualityObservation(providerRunRef, timestampMs),
      });
    }
    ...
    expect(coordinator.getSanitizedRecords()).toHaveLength(4);
    expect(coordinator.authorityBoundary.actualParticipantActionRequired).toBe(true);
    expect(coordinator.authorityBoundary.temporalSeparationIndependentlyVerified)
      .toBe(false);
    cam.close();
  });
  ...
  it('blocks session 2 until session 1 has exactly two slots', async () => {
    ...
    const coordinator = materializeGovernedBrowserDryRunCoordinatorFR249({
      ...
      jpegEncoder: {
        async encodeJpeg() {
          return new Uint8Array([0xff, 0xd8, 3, 0xff, 0xd9]);
        },
      },
    });
    ...
    coordinator.beginSession({
      sessionOrdinal: 1,
      issuedAt: '2026-09-22T22:40:00.000Z',
    });
    expect(() => coordinator.beginSession({
      sessionOrdinal: 2,
      issuedAt: '2026-09-22T22:41:00.000Z',
    })).toThrow(/exactly two capture slots/u);
    expect(() => coordinator.review()).toThrow(/exactly four recorded slots/u);
    cam.close();
  });
  ...
  it('retains a rejected slot without retrying or promoting empirical authority', async () => {
    ...
    const coordinator = materializeGovernedBrowserDryRunCoordinatorFR249({
      ...
      jpegEncoder: {
        async encodeJpeg() {
          return new Uint8Array([0xff, 0xd8, 4, 0xff, 0xd9]);
        },
      },
    });

    coordinator.beginSession({ sessionOrdinal: 1, issuedAt: '2026-09-22T22:50:00.000Z' });
    const slots = [
      { s: 1 as const, c: 1 as const, ts: 3001, at: '2026-09-22T22:50:10.000Z', fail: true },
    ...
    for (const slot of slots) {
      ...
      await coordinator.capture({
        ...
        operatorExecutionAttestation: executionAttestation(
          slot.c === 1 ? '2026-09-22T22:50:11.000Z' : '2026-09-22T22:50:21.000Z',
        ),
        operatorQualityObservation: qualityObservation(ref, slot.ts, {
          bilateralEyeContoursVisuallyResolvable: !slot.fail,
        }),
      });
    }
    ...
    for (const captureOrdinal of [1, 2] as const) {
      ...
      await coordinator.capture({
        challengeIssuedAt:
          captureOrdinal === 1
            ? '2026-09-22T23:00:10.000Z'
            : '2026-09-22T23:00:20.000Z',
        trigger: { timestampMs: ts, providerRunRef: ref },
        operatorExecutionAttestation: executionAttestation(
          captureOrdinal === 1
            ? '2026-09-22T23:00:11.000Z'
            : '2026-09-22T23:00:21.000Z',
        ),
        operatorQualityObservation: qualityObservation(ref, ts),
      });
    }
