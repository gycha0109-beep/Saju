...
async function valid105(): Promise<FiveOfficerMethodologySourceRebindReviewFR105V1> {
  const fr66 = await runPhotoToLipsContourNeutralSurfaceFR66({
    ...
    providerRunRef: 'fr105:hardening',
...
function forged(
  ...
  return { ...source, ...patch } as unknown as FiveOfficerMethodologySourceRebindReviewFR105V1;
}

describe('FR105 methodology source rebind hardening', () => {
  it('rejects methodology, metric, threshold, criterion, claim, and registry authority widening', async () => {
    const source = await valid105();
    const variants: readonly [FiveOfficerMethodologySourceRebindReviewFR105V1, RegExp][] = [
      [forged(source, { targetMethodology: { ...source.targetMethodology, reviewStatusAfter: 'reviewed' } }), /target methodology authority drift/u],
      ...
      [forged(source, { scanCheckedSourceCountAfter: 6 }), /authority widened/u],
      [forged(source, { authorityBoundary: { ...source.authorityBoundary, scanCheckedTextMeansMetricBinding: true } }), /fully fail-closed/u],
    ...
    for (const [candidate, pattern] of variants) {
      expect(() => validateFiveOfficerMethodologySourceRebindReviewFR105(candidate)).toThrow(pattern);
    }
  });
});
