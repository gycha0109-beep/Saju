...
async function artifact112(): Promise<FiveOfficerIntakePageVerificationReissueLineageContractReviewFR112V1> {
  const fr66 = await runPhotoToLipsContourNeutralSurfaceFR66({
    ...
    providerRunRef: 'fr112:hardening',
...
describe('FR112 page-verification reissue lineage contract hardening', () => {
  it('rejects lineage that would count the identity reissue as an independent verification', async () => {
    const result = await artifact112();
    const tampered = {
      ...result,
      lineageContractCandidate: {
        ...result.lineageContractCandidate,
        independentVerificationDelta: 1,
        childMayCountAsIndependentVerification: true,
      },
    } as unknown as FiveOfficerIntakePageVerificationReissueLineageContractReviewFR112V1;
    expect(() => validateFiveOfficerIntakePageVerificationReissueLineageContractReviewFR112(tampered))
      .toThrow(FaceAuthorityValidationError);
  });

  it('rejects turning the contract candidate into core schema, reissue, or persistence authority', async () => {
    const result = await artifact112();
    const tampered = {
      ...result,
      contractReview: {
        ...result.contractReview,
        coreSchemaExtensionAuthorized: true,
      ...
      execution: {
        ...
        directSourceRegistrySchemaChanged: true,
      },
    } as unknown as FiveOfficerIntakePageVerificationReissueLineageContractReviewFR112V1;
    expect(() => validateFiveOfficerIntakePageVerificationReissueLineageContractReviewFR112(tampered))
      .toThrow(FaceAuthorityValidationError);
  ...
  it('rejects replacing the selected registry relation with an inline mutation or semantic promotion', async () => {
    const result = await artifact112();
    const tampered = {
      ...result,
      contractReview: {
        ...result.contractReview,
        selectedPlacement: 'inline_record_field',
        inlineRecordMutationSelected: true,
      },
      execution: {
        ...result.execution,
        passagesPersisted: 1,
