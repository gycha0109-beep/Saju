...
export const FR267_CONTRACT_VERSION =
  'FR267-CONTROLLED-CAPTURE-GEOMETRY-SENSITIVITY-v1' as const;
...
export type FR267ControlledCondition =
  | 'baseline_eye_level'
  | 'low_angle'
  | 'high_angle';

export interface FR267ScalarSummary {
...
export interface FR267ConditionSummary {
  readonly condition: FR267ControlledCondition;
  readonly sourceRef: string;
  readonly sourceGeneratedAt: string;
  readonly captureCount: 4;
  readonly primaryMetricDegrees: FR267ScalarSummary;
  readonly geometry: {
    readonly lateralOrientationRadians: FR267ScalarSummary;
    readonly lateralOrientationDegrees: FR267ScalarSummary;
    readonly verticalOrientationRadians: FR267ScalarSummary;
    readonly verticalOrientationDegrees: FR267ScalarSummary;
    readonly relativeRotationFromFirstAcceptedCaptureRadians: FR267ScalarSummary;
    readonly relativeRotationFromFirstAcceptedCaptureDegrees: FR267ScalarSummary;
    readonly inPlaneLateralAxisOrientationRadians: FR267ScalarSummary;
    readonly inPlaneLateralAxisOrientationDegrees: FR267ScalarSummary;
    readonly poseUniformScaleComponent: FR267ScalarSummary;
    readonly screenFaceBoxWidthFraction: FR267ScalarSummary;
    readonly screenFaceBoxHeightFraction: FR267ScalarSummary;
    readonly screenFaceBoxAreaFraction: FR267ScalarSummary;
  };
}

export interface FR267BaselineRelativeContrast {
...
export interface FR267ControlledGeometrySensitivityReport {
  readonly schemaVersion: 'fr267-controlled-capture-geometry-sensitivity-report-v1';
  readonly artifactVersion: '0.1.0';
  readonly contractVersion: typeof FR267_CONTRACT_VERSION;
  ...
  readonly conditions: readonly [
    FR267ConditionSummary,
    FR267ConditionSummary,
    FR267ConditionSummary,
  ];
  readonly baselineRelativeContrasts: readonly [
    FR267BaselineRelativeContrast,
    FR267BaselineRelativeContrast,
  ];
...
export interface FR267SameFrameEyeTiltDiagnosticEvidence {
  readonly schemaVersion: 'fr267-same-frame-eye-tilt-diagnostic-evidence-v1';
  readonly artifactVersion: '0.1.0';
  readonly contractVersion: typeof FR267_CONTRACT_VERSION;
  readonly authorityState:
...
export interface FR267SameFrameEyeTiltDiagnosticSlot {
  readonly schemaVersion: 'fr267-same-frame-eye-tilt-diagnostic-slot-v1';
...
export interface FR267SameFrameEyeTiltDiagnosticBundle {
  readonly schemaVersion: 'fr267-same-frame-eye-tilt-diagnostic-bundle-v1';
  readonly artifactVersion: '0.1.0';
  readonly contractVersion: typeof FR267_CONTRACT_VERSION;
  readonly authorityState:
    'same_frame_eye_tilt_diagnostic_scalar_only_no_metric_replacement';
  readonly generatedAt: string;
  readonly slots: readonly FR267SameFrameEyeTiltDiagnosticSlot[];
  readonly descriptiveSummary: {
    readonly acceptedDiagnosticCount: number;
    readonly screenSpaceEyeOuterCornerTiltMeanDegrees: FR267ScalarSummary | null;
    readonly fr76CanonicalMetricEyeOuterCornerTiltMeanDegrees: FR267ScalarSummary | null;
    readonly screenMinusFr76Degrees: FR267ScalarSummary | null;
  };
  readonly persistenceBoundary: {
...
function summary(values: readonly number[]): FR267ScalarSummary {
...
function degreesSummary(values: readonly number[]): FR267ScalarSummary {
...
function contrast(
  baseline: FR267ConditionSummary,
  condition: FR267ConditionSummary,
): FR267BaselineRelativeContrast {
...
export function analyzeControlledCaptureGeometrySensitivityFR267(input: {
  readonly generatedAt: string;
  readonly conditions: readonly {
    readonly condition: FR267ControlledCondition;
    readonly sourceRef: string;
...
}): FR267ControlledGeometrySensitivityReport {
  ...
  return Object.freeze({
    schemaVersion: 'fr267-controlled-capture-geometry-sensitivity-report-v1' as const,
    artifactVersion: '0.1.0' as const,
    contractVersion: FR267_CONTRACT_VERSION,
    authorityState:
...
export function deriveSameFrameEyeTiltDiagnosticFR267(
  observation: FR257EphemeralGeometryObservation,
): FR267SameFrameEyeTiltDiagnosticEvidence {
  ...
  return Object.freeze({
    schemaVersion: 'fr267-same-frame-eye-tilt-diagnostic-evidence-v1' as const,
    artifactVersion: '0.1.0' as const,
    contractVersion: FR267_CONTRACT_VERSION,
    authorityState:
...
export function createSameFrameEyeTiltDiagnosticCollectorFR267():
FR267SameFrameEyeTiltDiagnosticCollector {
  const pending = new Map<string, FR267SameFrameEyeTiltDiagnosticEvidence>();
  const observe = (observation: FR257EphemeralGeometryObservation): void => {
    ...
    pending.set(
      ...
      deriveSameFrameEyeTiltDiagnosticFR267(observation),
    );
  };
  return Object.freeze({
    observe,
...
export function bindSameFrameEyeTiltDiagnosticSlotFR267(input: {
  readonly record: FR243DryRunCaptureExecutionRecord;
  readonly providerRunRef: string;
  readonly evidence: FR267SameFrameEyeTiltDiagnosticEvidence | null;
}): FR267SameFrameEyeTiltDiagnosticSlot {
  if (input.record.resultStatus === 'accepted_for_dry_run_mechanics_only') {
    if (
      ...
      fail('accepted FR243 capture requires exact FR267 same-frame evidence.');
    }
    return Object.freeze({
  ...
  if (input.evidence !== null || input.record.primaryMetric !== null) {
    fail('rejected capture must not retain FR267 scalar diagnostic evidence.');
  }
  return Object.freeze({
    schemaVersion: 'fr267-same-frame-eye-tilt-diagnostic-slot-v1' as const,
    sessionOrdinal: input.record.sessionOrdinal,
...
export function buildSameFrameEyeTiltDiagnosticBundleFR267(input: {
  readonly generatedAt: string;
  readonly slots: readonly FR267SameFrameEyeTiltDiagnosticSlot[];
}): FR267SameFrameEyeTiltDiagnosticBundle {
  exactIso(input.generatedAt);
  if (input.slots.length !== 4) fail('FR267 diagnostic bundle requires four slots.');
  const expected = ['1:1', '1:2', '2:1', '2:2'];
  input.slots.forEach((slot, index) => {
    if (`${slot.sessionOrdinal}:${slot.captureOrdinal}` !== expected[index]) {
      fail('FR267 diagnostic slots must remain ordered 1:1,1:2,2:1,2:2.');
    }
  });
  ...
  return Object.freeze({
    schemaVersion: 'fr267-same-frame-eye-tilt-diagnostic-bundle-v1' as const,
    artifactVersion: '0.1.0' as const,
    contractVersion: FR267_CONTRACT_VERSION,
    authorityState:
      'same_frame_eye_tilt_diagnostic_scalar_only_no_metric_replacement' as const,
