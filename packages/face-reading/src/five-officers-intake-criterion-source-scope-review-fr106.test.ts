...
async function artifact106() {
  const fr66 = await runPhotoToLipsContourNeutralSurfaceFR66({
    ...
    providerRunRef: 'fr106:criterion-scope',
  ...
  return reviewFiveOfficerIntakeCriterionSourceScopeFR106(fr105);
}

describe('FR106 Five Officers intake criterion source scope review', () => {
  it('admits an intake criterion research-unit candidate from one scan-checked source slot', async () => {
    const result = await artifact106();
    expect(result.criterionSourceScope.criterionCount).toBe(5);
    expect(result.criterionSourceScope.uniqueSourceRefs).toEqual(['passage.shenxiang.five_officers.intake']);
    expect(result.criterionSourceScope.everyCriterionUsesOnlyIntakePassage).toBe(true);
    expect(result.criterionSourceScope.scanCheckedCriterionSourceAvailable).toBe(true);
    expect(result.criterionSourceScope.criterionBundleSourceScopedIndependentlyOfMapping).toBe(true);
    expect(result.criterionSourceScope.intakeCriterionResearchUnitCandidateAdmitted).toBe(true);
  });

  it('retains the historical mapping dependency on the full intake officer definition', async () => {
    const result = await artifact106();
    expect(result.intakeOfficerDefinition.historicalSourceRefs).toEqual([
      'passage.shenxiang.five_officers.mapping',
      'passage.shenxiang.five_officers.intake',
    ]);
    expect(result.intakeOfficerDefinition.historicalMappingDependencyPresent).toBe(true);
    expect(result.intakeOfficerDefinition.mappingDependencyRemoved).toBe(false);
    expect(result.intakeOfficerDefinition.fullOfficerDefinitionDecompositionAuthorized).toBe(false);
  });

  it('records direct passage self-naming without using it to delete historical dependencies', async () => {
    const result = await artifact106();
    expect(result.directPassageSemantics.chapterSelfNamesOfficer).toBe(true);
    expect(result.directPassageSemantics.textSelfNamesOfficerFormation).toBe(true);
    expect(result.directPassageSemantics.exactTextMatchToScanCheckedOverlay).toBe(true);
    expect(result.directPassageSemantics.selfNamingMeansMappingDependencyRemoval).toBe(false);
  });

  it('does not issue a methodology definition, metric binding, threshold, criterion state, or traditional authority', async () => {
    const result = await artifact106();
    expect(result.criterionMethodologyDefinitionsIssued).toBe(0);
    ...
    expect(result.recommendedNextFrontier).toBe('intake_criterion_methodology_definition_review');
    expect(validateFiveOfficerIntakeCriterionSourceScopeReviewFR106(result)).toBe(result);
  });
});
