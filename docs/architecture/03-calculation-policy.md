...
```text
unknownBirthTime.mode = preserve-unknown
```

결과:

- 시주 = unavailable
- 시주가 필요한 deterministic fact = unavailable
- 시주가 필요한 interpretation rule = skipped
- 사용자에게 결과 completeness를 표시할 수 있음

향후 “가능한 12개 시주 모두 분석” 같은 기능을 제공한다면 이는 별도 feature/policy로 설계한다. 기본 계산을 임의값으로 오염시키지 않는다.
...
9. 시간 불명에서 partial calculation 지원 범위
