...
ROOT=Path('acquisition-choi-misook-2023'); ROOT.mkdir(exist_ok=True)
AUTHOR='최미숙'; YEAR='2023'
INSTITUTION='공주대학교 대학원'; SCHOOL_SIGNALS=('공주대학교 대학원','국립공주대학교 대학원')
TITLE_CORE='淸代 命理學 十星의 體用論的 解析에 관한 硏究'
BASE_TITLE='淸代 命理學 十星의 體用論的 解析에 관한 硏究 : 『命理約言』·『滴天髓闡微』 十星論의 生剋制化 樣相을 中心으로'
UA='Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Choi-Misook-2023-public-acquisition)'
...
assert card and detail_meta is not None and detail_raw is not None,'exact Choi Misook 2023 RISS card not resolved'
...
sig={'controlLiteralPresent':card['control'] in text,'titleLiteralPresent':all(x in text for x in ['淸代','命理學','十星','體用論的','解析']),'authorLiteralPresent':AUTHOR in text,'yearLiteralPresent':YEAR in text,'observedTIds':ids}; report['detailIdentitySignals']=sig
...
dm,dr=fetch(noredir,du,detail_meta['finalUrl'] or card['siteAuthoredDetailUrl']); pdf=ROOT/'choi-misook-2023.pdf'; external=[]
...
if pdf.exists():
    ...
    txt=''.join([f'\n===== PHYSICAL_PDF_PAGE_{i} =====\n{p.extract_text() or ""}' for i,p in enumerate(reader.pages,1)]); (ROOT/'choi-misook-2023.txt').write_text(txt,encoding='utf-8')
