...
def fetch(url: str, timeout: int = 60, referer: str | None = None) -> dict[str, object]:
    headers = {
        'User-Agent': UA,
...
def active_non_drm_redirect(html: str, path: str | None) -> bool:
    return bool(path and re.search(r"(?is)drm\s*==\s*['\"]N['\"].{0,3000}location\.replace\s*\(\s*context\s*\+", html))
...
report: dict[str, object] = {
    'candidate': {
        ...
        'nanetControl': 'KDMT1201802345',
        'priorAccessBoundaryPr': 371,
    },
    'policy': {
...
identity_markers = {'title': TITLE in riss_text or TITLE in riss_html, 'author': AUTHOR in riss_text or AUTHOR in riss_html, 'year': str(YEAR) in riss_text}
    ...
    if downloading_url:
        ...
        if dc_url:
            ...
            if permitted:
                ...
                pdf_saved, pdf_kind = save('na-hyukjin-2017-fulltext', pdf, pdf_body) if pdf_body else ('', '')
