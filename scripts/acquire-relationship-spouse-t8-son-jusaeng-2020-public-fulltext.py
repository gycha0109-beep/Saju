...
ROOT=Path('acquisition-son-jusaeng-2020'); ROOT.mkdir(exist_ok=True)
AUTHOR='손주생'; YEAR='2020'; SCHOOL_SIGNAL='경기대학교'
BASE_TITLE='四柱命理學의 格局과 用神에 대한 体用論的 硏究'
UA='Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Son-Jusaeng-2020-public-acquisition)'
...
def fetch(opener,u,ref=None,max_bytes=45_000_000):
    ...
    try:
        with opener.open(Request(u,headers=h),timeout=40) as r:
...
def norm_title(s):
    s=html.unescape(s).replace('體','体').replace('：',':').replace('–','-').replace('—','-')
...
def exact_card(base,text):
    ...
    for m in pat.finditer(text):
        ...
        if SCHOOL_SIGNAL not in etc and SCHOOL_SIGNAL not in block:continue
...
def form_block(text):
    ...
    e=text.find('</form>',m.end()); assert e>=0; return text[m.start():e+7]
def fields(block):
    out={}
...
report={'candidate':{'author':AUTHOR,'title':BASE_TITLE,'year':2020,'institution':SCHOOL_SIGNAL,'rissId':None,'control':None},'searchDiscovery':None,'detail':None,'detailIdentitySignals':None,'formTuple':None,'dispatcher':None,'dcollection':None,'pdfInspection':None,'renderedPageCount':0,'fullLengthPdfAcquired':False,'guessedOpaqueIdentifierCount':0,'loginBypass':False,'institutionAuthBypass':False,'paywallBypass':False,'drmRequestExecuted':False,'decryptionActionExecuted':False,'tlsVerificationDisabled':False,'crossSourceSemanticStitching':False,'semanticDisposition':'PUBLIC_ROUTE_INSPECTION_PENDING'}
terms=[BASE_TITLE,BASE_TITLE.replace('体','體'),'손주생 격국 용신 체용론']
...
assert card and detail_meta is not None and detail_raw is not None,'exact Son Jusaeng 2020 RISS card not resolved'
...
sig={'controlLiteralPresent':card['control'] in text,'titleLiteralPresent':('格局' in text and '用神' in text and ('体用' in text or '體用' in text)),'authorLiteralPresent':AUTHOR in text,'yearLiteralPresent':YEAR in text,'observedTIds':ids}; report['detailIdentitySignals']=sig
...
dm,dr=fetch(noredir,du,detail_meta['finalUrl'] or card['siteAuthoredDetailUrl']); pdf=ROOT/'son-jusaeng-2020.pdf'; external=[]
...
if pdf.exists():
    ...
    txt=''.join([f'\n===== PHYSICAL_PDF_PAGE_{i} =====\n{p.extract_text() or ""}' for i,p in enumerate(reader.pages,1)]); (ROOT/'son-jusaeng-2020.txt').write_text(txt,encoding='utf-8')
