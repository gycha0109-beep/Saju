...
ROOT = Path('acquisition-yu-myeongsuk-2024')
ROOT.mkdir(exist_ok=True)
AUTHOR = '유명숙'
YEAR = '2024'
SCHOOL_SIGNAL = '원광대학교'
BASE_TITLE = '중년여성의 부부위기 경험에 대한 명리학적 내러티브 연구'
FULL_TITLE = '중년여성의 부부위기 경험에 대한 명리학적 내러티브 연구 : 결정론과 자유의지를 중심으로'
UA = 'Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Yu-Myeongsuk-2024-public-acquisition)'
...
def fetch(opener, url: str, referer: str | None = None, max_bytes: int = 40_000_000):
...
def exact_card(base: str, text: str):
    ...
    pattern = re.compile(
        r'<p\s+class=["\']title["\']>\s*<a\s+href=["\']([^"\']*DetailView\.do\?[^"\']+)["\'][^>]*>(.*?)</a>\s*</p>\s*<p\s+class=["\']etc["\']>(.*?)</p>',
        re.I | re.S,
    )
    for match in pattern.finditer(text):
        ...
        if '원광대학교' not in etc and '원광대학교' not in block:
            continue
        ...
        if not control or not p_mat_type:
            continue
        return {
            'siteAuthoredDetailUrl': url,
            'title': title,
            'etc': etc,
            'control': control,
...
report = {
    'candidate': {'author': AUTHOR, 'title': FULL_TITLE, 'year': 2024, 'institution': SCHOOL_SIGNAL, 'rissId': None, 'control': None},
    'searchDiscovery': None,
    'detail': None,
    'detailIdentitySignals': None,
    'formTuple': None,
    'dispatcher': None,
    'dcollection': None,
    'pdfInspection': None,
...
terms = [FULL_TITLE, BASE_TITLE, '유명숙 부부위기 명리학 내러티브']
searches = []
card = None
detail_meta = detail_raw = None
for index, term in enumerate(terms, 1):
    search_url = 'https://www.riss.kr/search/Search.do?' + urlencode({
        'isDetailSearch': 'N',
        'searchGubun': 'true',
        'viewYn': 'OP',
        'query': term,
...
assert card and detail_meta is not None and detail_raw is not None, 'exact Yu Myeongsuk 2024 thesis search-result card not resolved'
assert detail_meta['status'] == 200
report['searchDiscovery'] = {
    'searches': searches,
    'card': card,
    'resolutionRule': 'EXACT_SITE_AUTHORED_RESULT_CARD_TITLE_AUTHOR_YEAR_SCHOOL_AND_LITERAL_DETAIL_ROUTE',
}
...
signals = {
    ...
    'titleLiteralPresent': ('중년여성' in detail_text and '부부위기' in detail_text and '명리학적' in detail_text),
...
for index, url in enumerate(script_urls, 1):
    try:
        ...
        if 'fulltextDownload' in script_text or 'FullTextDownload.do' in script_text:
...
pdf_path = ROOT / 'yu-myeongsuk-2024.pdf'
...
report['dispatcher'] = {
    ...
    'disposition': 'RISS_DISPATCHER_AUTHORED_DCOLLECTION_ROUTE' if external_urls else 'RISS_DISPATCHER_NO_RELEVANT_PUBLIC_BODY_ROUTE_OBSERVED',
}
    ...
    try:
        ...
        if entry_meta['pdfMagic'] or 'application/pdf' in entry_meta['contentType'].lower():
            ...
            for url in dcollection_candidates(entry_meta['finalUrl'] or authored, entry_text, expected_host)[:35]:
                ...
                dcollection['followed'].append({'url': url, 'response': file_meta})
                if file_meta['pdfMagic'] or 'application/pdf' in file_meta['contentType'].lower():
                    pdf_path.write_bytes(file_raw)
                    dcollection['directPdfAcquired'] = True
                    break
        dcollection['disposition'] = 'RISS_AUTHORED_DCOLLECTION_PUBLIC_PDF_ACQUIRED' if dcollection['directPdfAcquired'] else 'RISS_AUTHORED_DCOLLECTION_NO_DIRECT_PDF_OBSERVED'
    except URLError as exc:
        if isinstance(exc.reason, ssl.SSLCertVerificationError):
            dcollection['strictTlsError'] = f'{type(exc.reason).__name__}: {exc.reason}'
            dcollection['disposition'] = 'RISS_AUTHORED_DCOLLECTION_TLS_CERT_VERIFICATION_BOUNDARY_STOP_NO_BYPASS'
        else:
            raise
    report['dcollection'] = dcollection

if pdf_path.exists():
    pdf_bytes = pdf_path.read_bytes()
    reader = PdfReader(str(pdf_path))
    assert not reader.is_encrypted, 'encrypted PDF is not admitted'
    assert len(reader.pages) >= 100, 'PDF too short for full-thesis assumption; fail closed'
    text_parts = []
    for page_number, page in enumerate(reader.pages, 1):
        text_parts.append(f'\n===== PHYSICAL_PDF_PAGE_{page_number} =====\n{page.extract_text() or ""}')
    extracted = ''.join(text_parts)
    (ROOT / 'yu-myeongsuk-2024.txt').write_text(extracted, encoding='utf-8')
    render_dir = ROOT / 'rendered-pages'
    render_dir.mkdir(exist_ok=True)
    document = fitz.open(str(pdf_path))
    for page_index in range(document.page_count):
        page = document.load_page(page_index)
        pixmap = page.get_pixmap(matrix=fitz.Matrix(1.35, 1.35), colorspace=fitz.csGRAY, alpha=False)
        pixmap.save(str(render_dir / f'page-{page_index + 1:04d}.jpg'), jpg_quality=65)
    report['pdfInspection'] = {
        'sha256': sha256(pdf_bytes),
        'bytes': len(pdf_bytes),
        'pages': len(reader.pages),
        'encrypted': False,
        'textChars': len(extracted),
    }
    report['renderedPageCount'] = document.page_count
...
assert report['tlsVerificationDisabled'] is False
assert not report['crossSourceSemanticStitching']
