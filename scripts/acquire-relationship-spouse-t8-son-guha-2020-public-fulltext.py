...
ROOT=Path('acquisition-son-guha-2020'); ROOT.mkdir(exist_ok=True)
AUTHOR='손구하'; YEAR='2020'
INSTITUTION='동방문화대학원대학교'; SCHOOL_SIGNALS=('동방문화대학원대학교','東方文化大學院大學校')
TITLE_CORE='조선시대 명과학(命課學)과 단건업(段建業)의 맹파명리(盲波命理)에 관한 연구'
BASE_TITLE='조선시대 명과학(命課學)과 단건업(段建業)의 맹파명리(盲波命理)에 관한 연구'
UA='Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Son-Guha-2020-public-acquisition)'
...
assert card and detail_meta is not None and detail_raw is not None,'exact Son Guha 2020 RISS card not resolved'
...
sig={'controlLiteralPresent':card['control'] in text,'titleLiteralPresent':all(x in text for x in ['조선시대','명과학','단건업','맹파명리','연구']),'authorLiteralPresent':AUTHOR in text,'yearLiteralPresent':YEAR in text,'observedTIds':ids}; report['detailIdentitySignals']=sig
...
dm,dr=fetch(noredir,du,detail_meta['finalUrl'] or card['siteAuthoredDetailUrl']); pdf=ROOT/'son-guha-2020.pdf'; external=[]
...
if pdf.exists():
    ...
    txt=''.join([f'\n===== PHYSICAL_PDF_PAGE_{i} =====\n{p.extract_text() or ""}' for i,p in enumerate(reader.pages,1)]); (ROOT/'son-guha-2020.txt').write_text(txt,encoding='utf-8')
