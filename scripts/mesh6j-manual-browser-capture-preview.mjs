...
const fr256PagePath = resolve(repoRoot, 'tools/face-geometry/capture/fr256-mouth-observation-operator.html');
const fr256ClientPath = resolve(repoRoot, 'tools/face-geometry/capture/fr256-mouth-observation-operator.mjs');
...
async function main() {
  ...
  const fr256PageTemplate = readFileSync(fr256PagePath, 'utf8');
  if (!fr256PageTemplate.includes('__MEDIAPIPE_ENTRY__')) {
    fail('FR256 operator page import-map placeholder is missing.');
  }
  const fr256PageHtml = fr256PageTemplate.replaceAll('__MEDIAPIPE_ENTRY__', importMapTarget);
  ...
  const requestHandler = (request, response) => {
    ...
    if (url.pathname === '/fr256' || url.pathname === '/fr256/' || url.pathname === '/fr256/index.html') {
      response.writeHead(200, {
        'content-type': 'text/html; charset=utf-8',
        'cache-control': 'no-store',
        'content-security-policy':
      ...
      response.end(fr256PageHtml);
      return;
    }

    if (url.pathname === '/fr256/operator.mjs') {
      sendFile(response, fr256ClientPath);
      return;
    }
  ...
  if (SMOKE || LAN_SMOKE) {
    try {
      const required = [
        ...
        '/face/observable-morphology-fr142-same-frame-observation-fr256.js',
        importMapTarget,
      ];
  ...
  if (LAN_MODE) {
    ...
    if (urls.length === 0) {
      ...
      for (const url of urls) {
        ...
        process.stdout.write('FR256 mouth observation capture: ' + url + 'fr256/\n');
      }
    }
    ...
    process.stdout.write('FR256 mouth observation capture surface: ' + base + '/fr256/\n');
  }
