...
EXPECTED={'control_no':'49df877f9b4f8a08ffe0bdc3ef48d419','p_mat_type':'be54d9b8bc7cdb09','p_submat_type':'f1a8c7a1de0e08b8','fulltext_kind':'a8cb3aaead67ab5b'}
UA='Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Shin-Jaeeok-2024-RISS-dispatch)'
...
if meta['pdfMagic'] or 'application/pdf' in meta['contentType'].lower():
    (ROOT/'shin-jaeeok-2024.pdf').write_bytes(b); out['directPdfAcquired']=True; out['disposition']='RISS_DISPATCHER_DIRECT_PDF_ACQUIRED'
else:
    rt=decode(b,meta['contentType']); (ROOT/'riss-fulltext-dispatch-response.html').write_text(rt,encoding='utf-8')
    us=urls(endpoint,rt); out['literalUrls']=us[:120]
    for u in us:
