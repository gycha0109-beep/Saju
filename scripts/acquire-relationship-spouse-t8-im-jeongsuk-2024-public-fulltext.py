...
ROOT=Path('acquisition-im-jeongsuk-2024'); ROOT.mkdir(exist_ok=True)
AUTHOR='임정숙'; YEAR='2024'; SCHOOL_SIGNAL='국제뇌교육종합대학원대학교'
BASE_TITLE='사주日支 유형과 궁합에 대한 인식이 부부갈등에 미치는 영향'
FULL_TITLE='사주日支 유형과 궁합에 대한 인식이 부부갈등에 미치는 영향 : 이혼의도를 포함하여'
UA='Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Im-Jeongsuk-2024-public-acquisition)'
...
def is_riss(u:str)->bool:return host(u) in {'www.riss.kr','riss.kr','m.riss.kr'}
...
def norm_title(s:str)->str:
    s=html.unescape(s).replace('日支','일지').replace('：',':')
...
def exact_card(base:str,text:str):
    ...
    for m in pat.finditer(text):
        ...
        if target not in nt or AUTHOR not in etc or YEAR not in etc:continue
        start=max(0,m.start()-1200); end=min(len(text),m.end()+4500); block=clean(text[start:end])
        if '국제뇌교육' not in etc and '국제뇌교육' not in block:continue
...
terms=[FULL_TITLE,BASE_TITLE,'임정숙 사주일지 궁합 부부갈등']; searches=[]; card=None; detail_meta=detail_raw=None
...
assert card and detail_meta is not None and detail_raw is not None,'exact Im Jeongsuk 2024 thesis search-result card not resolved'
...
sig={'controlLiteralPresent':card['control'] in text,'titleLiteralPresent':('부부갈등' in text and '궁합' in text and ('日支' in text or '일지' in text)),'authorLiteralPresent':AUTHOR in text,'yearLiteralPresent':YEAR in text,'observedTIds':ids}; report['detailIdentitySignals']=sig
...
pdf=ROOT/'im-jeongsuk-2024.pdf'; external=[]
...
if pdf.exists():
    ...
    txt=''.join([f'\n===== PHYSICAL_PDF_PAGE_{i} =====\n{p.extract_text() or ""}' for i,p in enumerate(r.pages,1)]); (ROOT/'im-jeongsuk-2024.txt').write_text(txt,encoding='utf-8'); report['pdfInspection']={'sha256':sha(b),'bytes':len(b),'pages':len(r.pages),'encrypted':False,'textChars':len(txt)}; report['fullLengthPdfAcquired']=True; report['semanticDisposition']='DIRECT_BODY_ACQUIRED_REQUIRES_RENDER_FIRST_REVIEW'
