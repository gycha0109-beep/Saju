...
TITLE_STEM = '命理學 六親論 宮'
TITLE_VARIANTS = [
    TITLE,
    '命理學 六親論 宮·星의 변화 사례 硏究',
    '명리학 육친론 궁성의 변화 사례 연구',
]
EXPECTED_YEAR = '2024'
EXPECTED_INSTITUTION_TOKENS = ('공주대학교', '公州大學校')
UA = 'Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Shin-Jaeeok-2024-discovery)'
...
def title_match(text: str) -> bool:
    n = normalized(text)
    return '命理學六親論宮' .lower() in n and '변화사례' in n and ('硏究'.lower() in n or '연구' in n)


def contexts(text: str) -> list[str]:
    decoded = html.unescape(text)
    positions = []
    for token in [AUTHOR, '六親論', '변화 사례', '변화사례']:
        positions.extend(m.start() for m in re.finditer(re.escape(token), decoded, re.I))
    out = []
    for pos in sorted(set(positions))[:40]:
        c = decoded[max(0, pos - 10000): min(len(decoded), pos + 18000)]
        if AUTHOR in c and title_match(c):
            c = re.sub(r'\s+', ' ', c).strip()[:40000]
            if c not in out:
                out.append(c)
    return out
...
def detail_candidates(text: str) -> list[dict[str, str]]:
    ...
    for m in pat.finditer(decoded):
        href = m.group(1)
        rec = {'href': href, 'control': m.group(2).lower()}
        if rec not in out:
            out.append(rec)
    return out
...
report = {
    'candidate': {
        ...
        'title': TITLE,
        'year': 2024,
        'institution': '공주대학교 일반대학원',
        'dbpiaNodeId': 'T16939654',
    },
    'searches': [],
    'exactRowCandidates': [],
