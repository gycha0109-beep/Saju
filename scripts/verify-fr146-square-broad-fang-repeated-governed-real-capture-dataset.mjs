...
const allowlistTestPath = 'packages/face-reading/src/five-officers-square-broad-fang-repeated-governed-real-capture-dataset-fr146-allowlist.test.ts';
const notePath = 'research/face-reading/fr146-square-broad-fang-repeated-governed-real-capture-dataset.md';

const runtime = readFileSync(runtimePath, 'utf8');
const test = readFileSync(testPath, 'utf8');
const allowlistTest = readFileSync(allowlistTestPath, 'utf8');
...
for (const [fragment, label] of [
  ...
  ['const REQUEST_KEYS = new Set([', 'strict top-level request allowlist'],
  ['const CAPTURE_KEYS = new Set([', 'strict per-capture allowlist'],
  ['request contains unauthorized field:', 'top-level unauthorized-field rejection'],
  ['unexpectedCaptureField', 'per-capture unauthorized-field rejection'],
...
for (const [fragment, label] of [
  ['rejects unauthorized top-level request fields before capture execution', 'top-level allowlist regression'],
  ['rejects unauthorized capture fields before any provider execution', 'per-capture allowlist regression'],
  ['expect(calls).toBe(0)', 'pre-provider rejection assertion'],
]) requireFragment(allowlistTest, fragment, label);
...
for (const [fragment, label] of [
  ...
  forbidFragment(allowlistTest, fragment, label);
