...
# This is the current public NANET institution-search result URL that visibly
# exposes the exact Park Hyeyoung target as item 58 with 원문보기/다운로드.
# It is a normal public search URL, not an opaque record identifier.
SEARCH_URL = (
...
UA = 'Mozilla/5.0 (compatible; Saju-Research-Acquisition/2.0; Park-Hyeyoung-NANET-public-only)'
...
def fetch(opener, rr, url: str, *, referer: str | None=None, limit: int=MAX):
    ...
    for attempt in range(1,4):
        rr.chain.clear()
        meta={'requestedUrl':url,'status':None,'finalUrl':None,'redirectChain':[], 'contentType':None,'bytes':0,'sha256':None,'attempt':attempt,'error':None}
        try:
            with opener.open(Request(url, headers=headers), timeout=40) as resp:
                b=resp.read(limit)
                meta.update(status=getattr(resp,'status',None),finalUrl=resp.geturl(),redirectChain=list(rr.chain),contentType=resp.headers.get('Content-Type',''),bytes=len(b),sha256=sha256(b))
                return meta,b,decode(b)
        except Exception as exc:
            meta['error']=f'{type(exc).__name__}: {exc}'; last=meta; time.sleep(attempt)
    return last,b'', ''
...
def script_urls(text: str, base: str, host: str) -> list[str]:
    ...
    for raw in re.findall(r'<script[^>]+src=["\']([^"\']+)', text, re.I):
        ...
        if (urlparse(u).hostname or '').lower()==host and u not in out: out.append(u)
    return out[:30]
...
def row_contexts(text: str) -> list[dict]:
    ...
    while True:
        ...
        calls=[]
        for pat in [r'viewDoc\([^\)]{0,800}\)',r'downloadDoc\([^\)]{0,800}\)',r'goDetail\([^\)]{0,800}\)',r'detailView\([^\)]{0,800}\)']:
...
def main():
    rr=RedirectRecorder(); opener=build_opener(HTTPCookieProcessor(CookieJar()),rr)
    rep={
        ...
        'paywallBypass':False,
        'search':{},'detail':{},'innerJs':{},'viewer':{},'docviewer':{},
        'fullLengthPdfAcquired':False,
    }
    sm,sb,st=fetch(opener,rr,SEARCH_URL)
    (OUT/'nanet-search.html').write_text(st,encoding='utf-8')
    occ=row_contexts(st)
    (OUT/'nanet-occurrence-contexts.json').write_text(json.dumps(occ,ensure_ascii=False,indent=2),encoding='utf-8')
    exact=[o for o in occ if o['authorObserved'] and o['yearObserved']]
    controls=sorted(set(c for o in exact for c in o['controls']))
    rep['search']={
        'meta':sm,'titleOccurrenceCount':len(occ),'exactRowOccurrenceCount':len(exact),
        'exactRowControls':controls,
        'exactRowCalls':[c for o in exact for c in o['calls']],
    }
    # Only a unique control visibly co-located with title+author+year is admissible.
    rep['exactControl']=controls[0] if len(controls)==1 else None
    rep['controlResolutionDisposition']='EXACT_PUBLIC_SEARCH_ROW_CONTROL_RESOLVED' if len(controls)==1 else 'NO_UNIQUE_EXACT_PUBLIC_SEARCH_ROW_CONTROL'
    ...
    if rep['exactControl']:
        ...
        rep['innerJs']={'meta':im,'viewerDispatcherObserved':dispatcher,'downloadLoginGateObserved':login_gate}
        # Never execute download. Anonymous viewer bootstrap only if detail authors exact view button and dispatcher is explicit.
        if rep['detail']['titleObserved'] and rep['detail']['authorObserved'] and rep['detail']['viewButtonObserved'] and dispatcher:
            vu=f'https://dl.nanet.go.kr/view/callViewer.do?controlNo={control}&orgId=dl&linkSysId=NADL'
            ...
            if dv:
                du=dv[0]; xm,xb,xt=fetch(opener,rr,du,referer=vu,limit=3_000_000)
                ...
                for token in ['VUE_APP_MORE_PATH','VUE_APP_USE_SEC','drm','aes-256-cbc','/docinfo/','/page/','/pagec/','/regDoc']:
                    if token.lower() in union.lower(): signals.append(token)
                protected=any(x.lower() in {'vue_app_more_path','vue_app_use_sec','drm','aes-256-cbc'} for x in signals)
                rep['docviewer']={
                    'url':du,'meta':xm,'scriptFetches':scripts,'protectedContentSignals':signals,
                    'disposition':'DRM_OR_DEDICATED_VIEWER_SIGNAL_OBSERVED_STOP_NO_REPLAY' if protected else 'PUBLIC_VIEWER_STATIC_ONLY_NO_CONTENT_REPLAY'
                }
    (OUT/'nanet-report.json').write_text(json.dumps(rep,ensure_ascii=False,indent=2),encoding='utf-8')
    print(json.dumps({
        ...
        'contentAssetActionExecuted':False,'drmRequestExecuted':False,'guessedOpaqueIdentifierCount':0,
