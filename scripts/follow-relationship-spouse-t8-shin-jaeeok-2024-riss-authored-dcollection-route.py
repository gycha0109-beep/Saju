...
EXPECTED='https://kongju.dcollection.net/common/orgView/200001003260'
ITEM='200001003260'
TITLE_SIGNAL='命理學 六親論 宮'
AUTHOR='신재억'
UA='Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Shin-Jaeeok-2024-dCollection)'
...
def allowed(url):
    ...
    return h in {'kongju.dcollection.net','www.riss.kr','riss.kr'}
...
def exact_redirect(src,loc):
    ...
    return u
def fetch_chain(opener,start,referer,max_hops=2):
...
def viewer_pdf_literal(viewer_url,text):
    # Prefer the site-authored `file=` query parameter; accept only a direct public_resource PDF path.
    qs=parse_qs(urlparse(viewer_url).query)
    vals=qs.get('file',[])
    for v in vals:
        u=urljoin(viewer_url,html.unescape(v))
        p=urlparse(u)
        if p.hostname=='kongju.dcollection.net' and p.path.startswith('/public_resource/pdf/') and p.path.lower().endswith('.pdf'):
            return u
...
em,eb,chain=fetch_chain(opener,EXPECTED,d['request']['requestedUrl'],2); out['entry']=em; out['redirectChain']=chain
if em['pdfMagic'] or 'application/pdf' in em['contentType'].lower():
    (ROOT/'shin-jaeeok-2024.pdf').write_bytes(eb); out['directPdfAcquired']=True; out['pdf']={**em,'source':EXPECTED}
else:
    ...
    for i,u in enumerate(candidates[:20],1):
        try:
            cm,cb,cchain=fetch_chain(opener,u,em['requestedUrl'],2)
            ...
            if cm['pdfMagic'] or 'application/pdf' in cm['contentType'].lower():
                (ROOT/'shin-jaeeok-2024.pdf').write_bytes(cb); out['directPdfAcquired']=True; out['pdf']={**cm,'source':u}; break
            ...
            if pu:
                ...
                if pm['pdfMagic'] or 'application/pdf' in pm['contentType'].lower():
                    (ROOT/'shin-jaeeok-2024.pdf').write_bytes(pb); out['directPdfAcquired']=True; out['pdf']={**pm,'source':pu}; break
