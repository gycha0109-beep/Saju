...
ROOT = Path('acquisition-kim-insun-2010'); ROOT.mkdir(exist_ok=True)
AUTHOR = '김인순'
TITLE = '명리학의 궁합론 연구'
TITLE_HAN = '命理學의 宮合論 硏究'
YEAR = '2010'
INSTITUTION = '동방대학원대학교'
UA = 'Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Kim-Insun-2010-public-acquisition)'
...
assert card and detail_meta is not None and detail_raw is not None, 'exact Kim Insun 2010 thesis search-result card not resolved'
...
pdf = ROOT / 'kim-insun-2010.pdf'; external = []
...
if pdf.exists():
    ...
    txt = ''.join([f'\n===== PHYSICAL_PDF_PAGE_{i} =====\n{p.extract_text() or ""}' for i, p in enumerate(r.pages, 1)]); (ROOT / 'kim-insun-2010.txt').write_text(txt, encoding='utf-8')
