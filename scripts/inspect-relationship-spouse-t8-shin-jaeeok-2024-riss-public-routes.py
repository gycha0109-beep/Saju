...
url=row['detailUrl']
m,b=fetch(url)
t=decode(b,m['contentType'])
(ROOT/'riss-detail.html').write_text(t,encoding='utf-8')
assert AUTHOR in t and TITLE_SIGNAL in t and '2024' in t
assert 'T16939654' in t
assert 'I804:44004-000000033581' in t
assert 'onclick="javascript:fulltextDownload();"' in t or "onclick='javascript:fulltextDownload();'" in t

form_tuple={
 'control_no':first([r'name=["\']control_no["\'][^>]*value=["\']([^"\']*)',r'value=["\']([^"\']*)["\'][^>]*name=["\']control_no["\']'],t),
 'p_mat_type':first([r'name=["\']p_mat_type["\'][^>]*value=["\']([^"\']*)',r'value=["\']([^"\']*)["\'][^>]*name=["\']p_mat_type["\']'],t),
 'p_submat_type':first([r'name=["\']p_submat_type["\'][^>]*value=["\']([^"\']*)',r'value=["\']([^"\']*)["\'][^>]*name=["\']p_submat_type["\']'],t),
 'fulltext_kind':first([r'name=["\']fulltext_kind["\'][^>]*value=["\']([^"\']*)',r'value=["\']([^"\']*)["\'][^>]*name=["\']fulltext_kind["\']'],t),
}
...
doc_control=first([r'name=["\']docControlNo["\'][^>]*value=["\'](\d+)',r'value=["\'](\d+)["\'][^>]*name=["\']docControlNo["\']'],t)
national=first([r'name=["\']nationalLibraryLocalBibno["\'][^>]*value=["\']([^"\']*)',r'value=["\']([^"\']*)["\'][^>]*name=["\']nationalLibraryLocalBibno["\']'],t)

script_urls=[]
for u in links(m['finalUrl'],t):
    h=(urlparse(u).hostname or '').lower()
    if h in {'www.riss.kr','riss.kr'} and (u.lower().endswith('.js') or '.js?' in u.lower()) and u not in script_urls:
...
for i,u in enumerate(script_urls[:45],1):
    try:
        sm,sb=fetch(u,m['finalUrl'],20,3_000_000)
        st=decode(sb,sm['contentType']); union+='\n'+st
        fetches.append({**sm,'sourceUrl':u,'error':None})
        if 'fulltextDownload' in st or 'originalCheck' in st:
            fn=ROOT/f'riss-script-{i:02d}.txt'; fn.write_text(st,encoding='utf-8')
            impl.append(fn.name)
...
assert out['contentDownloadExecuted'] is False
