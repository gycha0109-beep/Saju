...
ROOT=Path('acquisition-kim-giyong-2017'); ROOT.mkdir(exist_ok=True)
AUTHOR='김기용'; YEAR='2017'; SCHOOL_SIGNAL='경기대학교'
BASE_TITLE='관상12궁과 사주육친의 상관성 비교고찰'
UA='Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Kim-Giyong-2017-public-acquisition)'
...
report={'candidate':{'author':AUTHOR,'title':BASE_TITLE,'year':2017,'institution':SCHOOL_SIGNAL,'rissId':None,'control':None},'searchDiscovery':None,'detail':None,'detailIdentitySignals':None,'formTuple':None,'dispatcher':None,'dcollection':None,'pdfInspection':None,'renderedPageCount':0,'fullLengthPdfAcquired':False,'guessedOpaqueIdentifierCount':0,'loginBypass':False,'institutionAuthBypass':False,'paywallBypass':False,'drmRequestExecuted':False,'decryptionActionExecuted':False,'tlsVerificationDisabled':False,'crossSourceSemanticStitching':False,'semanticDisposition':'PUBLIC_ROUTE_INSPECTION_PENDING'}
terms=[BASE_TITLE,'관상12궁 사주육친 상관성 비교고찰','김기용 사주육친 관상12궁']
...
assert card and detail_meta is not None and detail_raw is not None,'exact Kim Giyong 2017 RISS card not resolved'
...
sig={'controlLiteralPresent':card['control'] in text,'titleLiteralPresent':('관상12궁' in text and '사주육친' in text and '상관성' in text),'authorLiteralPresent':AUTHOR in text,'yearLiteralPresent':YEAR in text,'observedTIds':ids}; report['detailIdentitySignals']=sig
...
dm,dr=fetch(noredir,du,detail_meta['finalUrl'] or card['siteAuthoredDetailUrl']); pdf=ROOT/'kim-giyong-2017.pdf'; external=[]
...
if pdf.exists():
    ...
    txt=''.join([f'\n===== PHYSICAL_PDF_PAGE_{i} =====\n{p.extract_text() or ""}' for i,p in enumerate(reader.pages,1)]); (ROOT/'kim-giyong-2017.txt').write_text(txt,encoding='utf-8')
