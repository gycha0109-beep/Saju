...
ROOT=Path('acquisition-lee-seongyeop-2013'); ROOT.mkdir(exist_ok=True)
AUTHOR='이성엽'; YEAR='2013'
INSTITUTION='경기대학교 문화예술대학원'; SCHOOL_SIGNALS=('경기대학교 문화예술대학원',)
TITLE_CORE='四柱命理의 宮位論에 관한 硏究'
BASE_TITLE='四柱命理의 宮位論에 관한 硏究'
UA='Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Lee-Seongyeop-2013-public-acquisition)'
...
def exact_card(base,text):
    ...
    for m in pat.finditer(text):
        ...
        if not any(s in etc or s in block for s in SCHOOL_SIGNALS):continue
...
assert card and detail_meta is not None and detail_raw is not None,'exact Lee Seongyeop 2013 RISS card not resolved'
...
sig={'controlLiteralPresent':card['control'] in text,'titleLiteralPresent':all(x in text for x in ['四柱命理','宮位論','硏究']),'authorLiteralPresent':AUTHOR in text,'yearLiteralPresent':YEAR in text,'observedTIds':ids}; report['detailIdentitySignals']=sig
...
dm,dr=fetch(noredir,du,detail_meta['finalUrl'] or card['siteAuthoredDetailUrl']); pdf=ROOT/'lee-seongyeop-2013.pdf'; external=[]
...
if pdf.exists():
    b=pdf.read_bytes(); reader=PdfReader(str(pdf)); assert not reader.is_encrypted; assert len(reader.pages)>=65,'PDF too short for complete-thesis assumption; fail closed'
    txt=''.join([f'\n===== PHYSICAL_PDF_PAGE_{i} =====\n{p.extract_text() or ""}' for i,p in enumerate(reader.pages,1)]); (ROOT/'lee-seongyeop-2013.txt').write_text(txt,encoding='utf-8')
