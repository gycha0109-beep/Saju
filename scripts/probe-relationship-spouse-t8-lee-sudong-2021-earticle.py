...
UA = 'Mozilla/5.0 (compatible; MyeongHa-Research-Acquisition/8.3; public-resource-verification)'
...
def navigation_urls(text: str, base: str) -> list[str]:
    raw_values: list[str] = []
    raw_values.extend(re.findall(r'https?://[^\s"\'<>]+', text, re.I))
    raw_values.extend(re.findall(r'(?:location(?:\.href)?|window\.location)\s*=\s*["\']([^"\']+)', text, re.I))
    raw_values.extend(re.findall(r'<meta[^>]+http-equiv=["\']?refresh["\']?[^>]+content=["\'][^"\']*url=([^"\';>]+)', text, re.I))
    raw_values.extend(re.findall(r'(?:href|src|action)=["\']([^"\']+)["\']', text, re.I))
    out: list[str] = []
    for raw in raw_values:
        u = urljoin(base, raw.replace('&amp;', '&').strip())
        if u not in out:
            out.append(u)
    return out
...
def inspect_pdf(data: bytes, source_url: str, label: str) -> dict:
    rec: dict = {'sourceUrl': source_url, 'sha256': hashlib.sha256(data).hexdigest(), 'bytes': len(data), 'pageCount': 0, 'encrypted': None, 'textHealthy': False, 'keywordHits': [], 'renderedPages': [], 'error': None}
    try:
        reader = PdfReader(io.BytesIO(data)); rec['pageCount'] = len(reader.pages); rec['encrypted'] = bool(reader.is_encrypted)
        hit_pages: set[int] = set(); korean = 0
        for i, page in enumerate(reader.pages, 1):
            text = (page.extract_text() or '').replace('\x00', ' '); korean += len(re.findall(r'[가-힣]', text))
            lines = [x.strip() for x in text.splitlines() if x.strip()]; snippets: list[str] = []
            for n, line in enumerate(lines):
                if any(k in line for k in KEYWORDS):
                    lo=max(0,n-2); hi=min(len(lines),n+3); snippets.append(' / '.join(lines[lo:hi])[:1800])
            if snippets:
                hit_pages.add(i); rec['keywordHits'].append({'physicalPdfPage':i,'snippets':snippets[:10]})
        rec['textHealthy'] = korean >= 150; n = rec['pageCount']
        if n <= 3:
        ...
        try:
            for p in targets:
                ...
                f=OUT/f'rendered-earticle-{label}-p{p:03d}.png'; pix.save(f)
...
def safe_public_candidate(url: str) -> bool:
    low = url.lower()
    if any(k in low for k in ('login','account','connect','purchase','buy','payment','member','auth')):
        return False
    return any(k in low for k in ('download','original','fulltext','.pdf','file1.earticle.net','riss.kr','nrf.re.kr'))
...
def main() -> int:
    ...
    report: dict = {
        ...
        'title':TITLE,'doi':DOI,'searchUrl':SEARCH,'targetViewUrl':TARGET,
        'search':{},'targetView':{},'targetBodySample':None,'targetSignals':[],'targetNavigationUrls':[],
        'explicitPublicFulltextLinks':[],'accessControlObserved':False,'probes':[],'pdfs':[],'fullLengthPdfAcquired':False,
    }

    search_meta,search_data=fetch(opener,rd,SEARCH); report['search']=search_meta
    search_text=decode(search_data) if search_data else ''
    assert TITLE in search_text; assert 'Public/View/5/3688156' in search_text

    target_meta,target_data=fetch(opener,rd,TARGET,search_meta.get('finalUrl') or SEARCH); report['targetView']=target_meta
    if is_pdf(target_meta,target_data):
        (PRIVATE/'earticle-target.pdf').write_bytes(target_data); report['pdfs'].append(inspect_pdf(target_data,target_meta.get('finalUrl') or TARGET,'target'))
    elif target_data:
        text=decode(target_data); base=target_meta.get('finalUrl') or TARGET; low=text.lower()
        report['targetBodySample']=compact(text,7000)
        report['targetSignals']=contexts(text,(TITLE,DOI,'원문보기','다운로드','무료','NRF','file1.earticle.net','PDF','로그인','소속기관','기관인증','구매','결제','IP대역','location','refresh','window.open'),50)
        nav=navigation_urls(text,base); report['targetNavigationUrls']=nav[:30]
        report['accessControlObserved']=any(m.lower() in low for m in ACCESS_MARKERS)
        explicit=[u for u in nav if safe_public_candidate(u)]
        for u in hrefs(text,base):
            if safe_public_candidate(u) and u not in explicit: explicit.append(u)
        report['explicitPublicFulltextLinks']=explicit[:12]

        if not report['accessControlObserved']:
            for idx,u in enumerate(explicit[:8],1):
                m,b=fetch(opener,rd,u,base); rec=dict(m); rec['origin']='exact-target-explicit-public-navigation'
                if is_pdf(m,b):
                    (PRIVATE/f'earticle-explicit-{idx:02d}.pdf').write_bytes(b); report['pdfs'].append(inspect_pdf(b,m.get('finalUrl') or u,f'explicit-{idx:02d}'))
                elif b:
                    t=decode(b); rec['signals']=contexts(t,(TITLE,DOI,'로그인','소속기관','기관인증','구매','결제','IP대역','PDF','download','location'),20); rec['bodySample']=compact(t,5000)
                    if any(marker.lower() in t.lower() for marker in ACCESS_MARKERS): report['accessControlObserved']=True
                report['probes'].append(rec)
                if report['accessControlObserved']: break
