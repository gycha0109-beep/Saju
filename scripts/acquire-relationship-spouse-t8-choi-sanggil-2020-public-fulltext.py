...
ROOT=Path('acquisition-choi-sanggil-2020'); ROOT.mkdir(exist_ok=True)
AUTHOR='최상길'; YEAR='2020'; SCHOOL_SIGNAL='동방문화대학원대학교'
BASE_TITLE='사주명리에 있어서 천간별 十星의 주요 특성 연구 : 천간별 오행의 특성과 十星의 상관관계를 중심으로'
UA='Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Choi-Sanggil-2020-public-acquisition)'
...
terms=[BASE_TITLE,'사주명리에 있어서 천간별 十星의 주요 특성 연구','최상길 천간별 십성 주요 특성 연구']
...
assert card and detail_meta is not None and detail_raw is not None,'exact Choi Sanggil 2020 RISS card not resolved'
...
sig={'controlLiteralPresent':card['control'] in text,'titleLiteralPresent':('천간별' in text and '十星' in text and '주요 특성' in text),'authorLiteralPresent':AUTHOR in text,'yearLiteralPresent':YEAR in text,'observedTIds':ids}; report['detailIdentitySignals']=sig
...
dm,dr=fetch(noredir,du,detail_meta['finalUrl'] or card['siteAuthoredDetailUrl']); pdf=ROOT/'choi-sanggil-2020.pdf'; external=[]
...
if pdf.exists():
    ...
    txt=''.join([f'\n===== PHYSICAL_PDF_PAGE_{i} =====\n{p.extract_text() or ""}' for i,p in enumerate(reader.pages,1)]); (ROOT/'choi-sanggil-2020.txt').write_text(txt,encoding='utf-8')
