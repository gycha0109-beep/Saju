...
ROOT=Path('acquisition-lee-donwoo-2019'); ROOT.mkdir(exist_ok=True)
AUTHOR='이돈우'; YEAR='2019'; INSTITUTION='국제뇌교육종합대학원대학교'
BASE_TITLE='古典 『命理約言』 이론 硏究'
UA='Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Lee-Donwoo-2019-public-acquisition)'
...
def exact_card(base,text):
    ...
    for m in pat.finditer(text):
        ...
        if INSTITUTION not in etc and INSTITUTION not in block:continue
...
terms=[BASE_TITLE,'古典 命理約言 이론 硏究','이돈우 명리약언 이론 연구']
...
assert card and detail_meta is not None and detail_raw is not None,'exact Lee Donwoo 2019 RISS card not resolved'
...
sig={'controlLiteralPresent':card['control'] in text,'titleLiteralPresent':('命理約言' in text and '이론' in text and '硏究' in text),'authorLiteralPresent':AUTHOR in text,'yearLiteralPresent':YEAR in text,'observedTIds':ids}; report['detailIdentitySignals']=sig
...
dm,dr=fetch(noredir,du,detail_meta['finalUrl'] or card['siteAuthoredDetailUrl']); pdf=ROOT/'lee-donwoo-2019.pdf'; external=[]
...
if pdf.exists():
    ...
    txt=''.join([f'\n===== PHYSICAL_PDF_PAGE_{i} =====\n{p.extract_text() or ""}' for i,p in enumerate(reader.pages,1)]); (ROOT/'lee-donwoo-2019.txt').write_text(txt,encoding='utf-8')
