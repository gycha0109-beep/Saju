...
ROOT=Path('acquisition-park-byeonggeun-2021'); ROOT.mkdir(exist_ok=True)
AUTHOR='박병근'
TITLE='宮合의 吉凶 분석방법 연구'
YEAR='2021'
INSTITUTION='공주대학교 일반대학원'
UA='Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Park-Byeonggeun-2021-public-acquisition)'
...
assert ('宮合의 吉凶 분석방법 연구' in text or ('宮合' in text and '吉凶' in text and '분석방법' in text))
...
pdf=ROOT/'park-byeonggeun-2021.pdf'; external=[]
...
if pdf.exists():
    ...
    txt=''.join(chunks); (ROOT/'park-byeonggeun-2021.txt').write_text(txt,encoding='utf-8')
    report['pdfInspection']={'sha256':sha(b),'bytes':len(b),'pages':len(r.pages),'encrypted':False,'textChars':len(txt)}
    report['fullLengthPdfAcquired']=len(r.pages)>=85
