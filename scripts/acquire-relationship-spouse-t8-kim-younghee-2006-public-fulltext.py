...
ROOT=Path('acquisition-kim-younghee-2006'); ROOT.mkdir(exist_ok=True)
AUTHOR='김영희'; TITLE='宮合理論硏究'; TITLE_ALT='궁합이론 연구'; YEAR='2006'; INSTITUTION='공주대학교 대학원'
UA='Mozilla/5.0 (compatible; SajuResearchEvidence/1.0; Kim-Younghee-2006-public-acquisition)'
...
def fetch(opener,url:str,referer:str|None=None,max_bytes:int=30_000_000):
    ...
    try:
        ...
        meta.update({'finalUrl':url,'status':e.code,'location':e.headers.get('Location'),'contentType':e.headers.get('Content-Type',''),'contentDisposition':e.headers.get('Content-Disposition',''),'bytes':len(raw),'sha256':sha(raw),'pdfMagic':raw.startswith(b'%PDF-'),'error':f'HTTPError: {e.code}'}); return meta,raw
...
def riss_detail_links(base:str,text:str)->list[str]:
    ...
    for u in links(base,text):
        if is_riss(u) and 'DetailView.do' in urlparse(u).path and parse_qs(urlparse(u).query).get('control_no') and u not in out:out.append(u)
...
def doc_form(text:str)->str:
    ...
    end=text.find('</form>',m.end()); assert end>=0; return text[m.start():end+7]
...
def exact_candidate_page(text:str)->bool:
    ...
    return re.sub(r'\s+','',TITLE) in compact or TITLE_ALT.replace(' ','') in compact or (('宮合' in text or '궁합' in text) and ('理論' in text or '理論' in text or '이론' in text) and ('硏究' in text or '연구' in text))
...
for si,surl in enumerate(search_urls,1):
    ...
    found=riss_detail_links(sm['finalUrl'] or surl,st); discovery.append({'searchUrl':surl,'response':sm,'siteAuthoredDetailLinkCount':len(found)})
    for u in found[:40]:
        request_u=normalize_http_url(u); dm,dr=fetch(normal,request_u,sm['finalUrl'] or surl,max_bytes=9_000_000); dt=decode(dr,dm['contentType'])
        if exact_candidate_page(dt):resolved_authored=u; resolved=request_u; detail_meta=dm; detail_raw=dr; break
    if resolved:break
...
text=decode(detail_raw,detail_meta['contentType']); (ROOT/'riss-detail.html').write_text(text,encoding='utf-8'); assert exact_candidate_page(text)
q=parse_qs(urlparse(resolved).query); control=(q.get('control_no') or [None])[0]; assert control; report['candidate']['control']=control
ids=sorted(set(re.findall(r'\bT\d{7,}\b',text))); report['candidate']['rissId']=ids[0] if len(ids)==1 else None; report['detail']=detail_meta
f=fields(doc_form(text)); assert f.get('control_no')==control and f.get('p_mat_type'); report['formTuple']={k:f.get(k) for k in ['control_no','p_mat_type','p_submat_type','fulltext_kind']}
observed=False
...
assert observed
f['loginFlag']='1'; f['content_page']=''; endpoint='https://www.riss.kr/search/download/FullTextDownload.do'; req=endpoint+'?'+urlencode(f); dm,dr=fetch(no_redirect,req,detail_meta['finalUrl'] or resolved)
pdf=ROOT/'kim-younghee-2006.pdf'; external=[]
...
if pdf.exists():
    b=pdf.read_bytes(); r=PdfReader(str(pdf)); assert not r.is_encrypted; txt=''.join([f'\n===== PHYSICAL_PDF_PAGE_{i} =====\n{p.extract_text() or ""}' for i,p in enumerate(r.pages,1)]); (ROOT/'kim-younghee-2006.txt').write_text(txt,encoding='utf-8'); report['pdfInspection']={'sha256':sha(b),'bytes':len(b),'pages':len(r.pages),'encrypted':False,'textChars':len(txt)}; report['fullLengthPdfAcquired']=len(r.pages)>=50; report['semanticDisposition']='DIRECT_BODY_ACQUIRED_REQUIRES_RENDER_FIRST_REVIEW'
