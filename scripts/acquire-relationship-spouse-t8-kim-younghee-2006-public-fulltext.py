...
AUTHOR='김영희'; TITLE='宮合理論硏究'; TITLE_ALT='궁합 이론 연구'; YEAR='2006'; INSTITUTION='공주대학교 대학원'
UA='Mozilla/5.0 (compatible; SajuResearchEvidence/1.3; Kim-Younghee-2006-public-acquisition)'
...
def exact_search_card(base:str,text:str):
    target_titles={norm_title(TITLE),norm_title(TITLE_ALT)}
    ...
    for m in pat.finditer(text):
        ...
        start=max(0,m.start()-1500); end=min(len(text),m.end()+6000); block=text[start:end]
        # Require thesis type/school signal from the same result block, not page-wide references.
        block_text=clean(block)
        if '국내석사' not in block_text and '석사' not in block_text:continue
        if '公州大學校' not in block_text and '공주대학교' not in block_text:continue
...
search_terms=[TITLE,TITLE_ALT,'김영희 궁합이론 연구']; discovery=[]; card=None; detail_meta=detail_raw=None
...
assert card and detail_meta is not None and detail_raw is not None,'exact Kim Younghee 2006 thesis search-result card not resolved'
...
# Detail page is confirmation of the exact card-authored route, not a new page-wide identity discovery step.
...
ids=sorted(set(re.findall(r'https://www\.riss\.kr/link\?id=(T\d+)',text)))
report['candidate']['rissId']=ids[0] if len(ids)==1 else None
signals={'controlLiteralPresent':card['control'] in text,'titleLiteralPresent':('宮合理論' in text or '궁합 이론' in text or '궁합이론' in text),'authorLiteralPresent':AUTHOR in text,'yearLiteralPresent':YEAR in text,'observedTIds':ids}
...
report['formTuple']=form_tuple
# If the exact card exposed fulltext args, require current detail form not to contradict them.
