...
describe('R084 false-positive interpretation patterns', () => {
  ...
  it('holds every Production assertion pending missing evidence', () => {
    ...
    expect(R084_FALSE_POSITIVE_PATTERNS.every((x) => x.evidenceRefs.length > 0)).toBe(true);
    expect(R084_FALSE_POSITIVE_PATTERNS.find((x) => x.id === 'FP08')?.evidenceRefs).toContain('R083:#1235');
    expect(R084_FALSE_POSITIVE_PATTERNS.find((x) => x.id === 'FP10')?.evidenceRefs).toContain('R083:#1235');
  });
