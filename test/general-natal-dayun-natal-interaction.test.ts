...
describe('R072 Dayun interaction with natal structure', () => {
  it('preserves five distinct natal-context interaction classes', () => {
    ...
    expect(R072_INTERACTION_CLASSES.filter((x) => x.provenanceKind === 'DIRECT_QUOTE_PAIR')).toHaveLength(1);
    expect(R072_INTERACTION_CLASSES.filter((x) => x.provenanceKind === 'PARAPHRASED_SOURCE_CLASS')).toHaveLength(4);
