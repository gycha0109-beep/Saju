...
describe('R082 historical/example calculation normalization', () => {
  ...
  it('normalizes eight examples without claiming independent recalculation', () => {
    ...
    expect(R082_NORMALIZED_EXAMPLES.find((x) => x.id === 'YUAN-SHIKAI-MAO-LUCK')).toEqual(expect.objectContaining({
      reportedBirthDate: '1859-09-16',
      birthDateEvidenceClass: 'CORROBORATED_BIOGRAPHICAL_RECORDS_NOT_PRIMARY_BIRTH_RECORD',
    }));
    expect(R082_NORMALIZED_EXAMPLES.find((x) => x.id === 'LI-GUOJIE-WEI-JIAXU')).toEqual(expect.objectContaining({
      modes: ['PARTIAL_BIRTH_DATA'],
      birthDateEvidenceClass: 'INSUFFICIENT_FOR_EXACT_DAYUN_START',
    }));
  });
