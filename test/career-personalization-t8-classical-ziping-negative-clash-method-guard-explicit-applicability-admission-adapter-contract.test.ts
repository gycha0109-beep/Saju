...
describe('Career T8 classical Zi-Ping B64 explicit-applicability research admission adapter', () => {
  test('admits only an explicitly applicable, content-addressed, identity-bound and guard-compatible proposal', () => {
    ...
    expect(decision.guardEvaluation).toEqual({ accepted: true, violationIds: [], violationCount: 0 });
  ...
  test('fails closed on missing applicability, content mismatch, identity mismatch and incomplete shape', () => {
    const valid = admittedCandidate();
    const missingApplicability =
      evaluateCareerT8ClassicalZipingNegativeClashMethodGuardResearchAdmissionCandidate({
        subjectType: valid.subjectType,
        subjectRef: valid.subjectRef,
        subjectContent: valid.subjectContent,
        proposalShape: valid.proposalShape,
      });
    expect(missingApplicability.rejectionReasonIds).toContain(
      'EXPLICIT_B64_APPLICABILITY_DECLARATION_REQUIRED',
    );

    const badHash = evaluateCareerT8ClassicalZipingNegativeClashMethodGuardResearchAdmissionCandidate({
      ...valid,
      subjectRef: { ...valid.subjectRef, contentHash: '0'.repeat(64) },
    });
    expect(badHash.rejectionReasonIds).toContain('SUBJECT_CONTENT_HASH_MISMATCH');

    const badIdentity = evaluateCareerT8ClassicalZipingNegativeClashMethodGuardResearchAdmissionCandidate({
      ...valid,
      subjectRef: { ...valid.subjectRef, id: 'RULE-B67-WRONG-ID' },
    });
    expect(badIdentity.subjectContentBound).toBe(true);
    expect(badIdentity.rejectionReasonIds).toContain('SUBJECT_IDENTITY_MISMATCH');
    ...
    expect(incompleteShape.proposalShape).toBeNull();
    expect(incompleteShape.guardEvaluation).toBeNull();
    expect(incompleteShape.rejectionReasonIds).toContain('COMPLETE_B64_PROPOSAL_SHAPE_REQUIRED');
  });

  test.each([
    ['resolvesSemanticEffectFromClashPresenceAlone', 'CLASH_PRESENCE_ALONE_MAY_NOT_RESOLVE_SEMANTIC_EFFECT'],
    ['assumesContextFreeUniformDamage', 'CONTEXT_FREE_UNIFORM_DAMAGE_MAY_NOT_BE_ASSUMED'],
    ['usesFixedNumericClashOffsetMultiplierOrScalar', 'FIXED_NUMERIC_CLASH_OFFSET_MULTIPLIER_OR_SCALAR_MAY_NOT_BE_INFERRED'],
    ['dropsSourceRequiredContextOrAffectedTargetRole', 'SOURCE_REQUIRED_CONTEXT_OR_AFFECTED_TARGET_ROLE_MAY_NOT_BE_DROPPED_FOR_CURRENT_METHOD_COMPATIBILITY'],
    ['flattensQualitativelyDivergentEffectClasses', 'QUALITATIVELY_DIVERGENT_SOURCE_OUTCOMES_MAY_NOT_BE_FLATTENED_TO_ONE_UNARY_EFFECT_CLASS'],
  ] as const)('rejects B64 violation %s', (field, expectedReason) => {
    const candidate = admittedCandidate();
    const decision = evaluateCareerT8ClassicalZipingNegativeClashMethodGuardResearchAdmissionCandidate({
      ...candidate,
  ...
  test('creates deterministic content-addressed decisions', () => {
    const first = evaluateCareerT8ClassicalZipingNegativeClashMethodGuardResearchAdmissionCandidate(admittedCandidate());
    const second = evaluateCareerT8ClassicalZipingNegativeClashMethodGuardResearchAdmissionCandidate(admittedCandidate());
...
describe('Career T8 classical Zi-Ping explicit-applicability admission adapter contract materialization', () => {
  test('materializes research-only contract and preserves all semantic and production boundaries', () => {
    const report =
      buildCareerPersonalizationT8ClassicalZipingNegativeClashMethodGuardExplicitApplicabilityAdmissionAdapterContract(
        acceptedB66(),
      );
    ...
    expect(report.adapterContract).toEqual(CAREER_T8_B67_EXPLICIT_APPLICABILITY_ADMISSION_ADAPTER_CONTRACT);
  ...
  test('opens only integration-readiness review and freezes sixteen controls', () => {
    const report =
      buildCareerPersonalizationT8ClassicalZipingNegativeClashMethodGuardExplicitApplicabilityAdmissionAdapterContract(
        acceptedB66(),
      );
  ...
  test('fails closed on tampered B66 content address', () => {
