...
import { createCareerAnnualReadingCandidateRegistry } from '../src/research/career-annual-reading-candidate.js';
...
const FIXED_CALCULATION_TIME = new Date('2026-09-04T02:00:00.000Z');
const FIXED_INTERPRETATION_TIME = new Date('2026-09-04T02:01:00.000Z');
const NARRATIVE_POLICY_VERSION = 'myeongha-career-annual-narrative-v1';
const NARRATIVE_POLICY_KEY = `myeongha-career-annual@${NARRATIVE_POLICY_VERSION}`;
...
function careerAnnualConsumedInputFingerprint(
  ...
  const entries = selectedCareerClaims(execution.claims, selectedClaimIds)
    .flatMap((claim) =>
  ...
  return `career_annual_consumed_input_${deterministicContentHash({
    domain: 'career',
...
function careerAnnualInterpretationSignature(
  execution: ReturnType<typeof runInterpretation>,
  selectedClaimIds: readonly string[],
): string {
  const selectedClaims = selectedCareerClaims(execution.claims, selectedClaimIds);
  if (selectedClaims.length === 0) throw new Error('Expected selected Career Annual claims.');
  ...
  return `career_annual_interpretation_${deterministicContentHash({
    domain: 'career',
    temporalScope: 'annual',
    claims: materials,
  })}`;
...
function buildObservation(
  ...
  const registry = createCareerAnnualReadingCandidateRegistry(FIXED_CALCULATION_TIME.toISOString());
  ...
  if (temporalContext === undefined || temporalContext.scope !== 'annual') {
    throw new Error(`Career Annual temporal context missing for ${caseId}.`);
  }
  ...
  if (composition.evidence === undefined) {
    throw new Error(`Career Annual evidence bundle missing for ${caseId}.`);
  }
  if (composition.selection.coverageState !== 'complete') {
    throw new Error(
      `Career Annual coverage was ${composition.selection.coverageState} for ${caseId}.`,
    );
  }
  ...
  const selectedClaims = selectedCareerClaims(
    execution.claims,
  ...
  if (
    ...
    throw new Error(`Career Annual T8/T9 composition missing for ${caseId}.`);
  }
  ...
  if (!fallback.validation.valid) {
    throw new Error(`Invalid Career Annual fallback for ${caseId}.`);
  }
  ...
  return {
    ...
    consumedInputFingerprint: careerAnnualConsumedInputFingerprint(
      execution,
      composition.selection.selectedClaimIds,
    ),
    interpretationSignature: careerAnnualInterpretationSignature(
      execution,
      composition.selection.selectedClaimIds,
    ),
...
describe('MyeongHa Career Annual synthetic corpus', () => {
  it('keeps calculation, consumed-input, interpretation, and narrative signatures deterministic', () => {
    const first = buildObservation('career-annual-determinism', 1, 9, 9);
    const repeated = buildObservation('career-annual-determinism', 1, 9, 9);
  ...
  it(
    'measures 500 actual Career Annual readings and rejects cross-semantic clones or false diversity',
    () => {
      ...
      outer: for (let month = 1; month <= 12; month += 1) {
        for (let day = 1; day <= 28; day += 1) {
          for (const hour of SYNTHETIC_HOURS) {
            ...
            const caseId = `career-annual-synthetic-${String(scannedBirthInputs).padStart(4, '0')}`;
      ...
      process.stdout.write(
        `CAREER_ANNUAL_SYNTHETIC_MEASUREMENT ${JSON.stringify({
