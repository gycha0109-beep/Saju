...
function request(month: number): ReadingRequest {
  return {
    requestId: `relationship-monthly-${month}`,
...
function execute(natal: CanonicalSajuSnapshot, month: number) {
  ...
  const registry = createRelationshipMonthlyReadingCandidateRegistry(NOW.toISOString());
...
describe('MyeongHa Relationship Monthly T9 reading candidate', () => {
  it('reuses Relationship Natal T8 without General Monthly or Relationship Annual T9 leakage', () => {
    ...
    expect(RELATIONSHIP_MONTHLY_READING_PACK.status).toBe('research');
    expect(RELATIONSHIP_MONTHLY_READING_METHODOLOGY.family).toBe('time_dynamics');
    ...
    expect(
      registry.rules.some(
        (rule) =>
          ...
          (rule.taxonomy.category === 'general' || rule.taxonomy.subcategory === 'annual'),
      ),
    ).toBe(false);
  });

  it('emits one personalized relationship activation for each before/after jeol segment from temporal facts', () => {
    ...
    for (const claim of themes) {
      expect(claim.taxonomy).toEqual({ tier: 'T9', category: 'relationship', subcategory: 'monthly' });
  ...
  it('makes relationship/monthly complete with Relationship Natal T8 plus Relationship Monthly T9 evidence', () => {
    ...
    expect(
      composition.evidence?.bundle.claims.some(
        (claim) => claim.taxonomy.tier === 'T8' && claim.taxonomy.category === 'relationship',
      ),
    ).toBe(true);
    expect(
      composition.evidence?.bundle.claims.some(
  ...
  it('is deterministic for the same month and changes monthly relationship semantics across adjacent months', () => {
    const natal = snapshot();
  ...
  it('emits resolved clashes only as bounded communication/expectation/boundary/pace adjustment pressure', () => {
    let found = false;
    for (let day = 1; day <= 12; day += 1) {
      const result = execute(snapshot(day), 9);
      const tensions = result.execution.claims.filter(
        (claim) => claim.state === 'active' && claim.claimType === RELATIONSHIP_MONTHLY_TENSION_CLAIM_TYPE,
      );
  ...
  it('keeps monthly themes but never fabricates hour relation evidence when birth time is unknown', () => {
    ...
    expect(
      result.execution.claims.some(
        (claim) =>
          claim.state === 'active' &&
          claim.claimType === RELATIONSHIP_MONTHLY_TENSION_CLAIM_TYPE &&
