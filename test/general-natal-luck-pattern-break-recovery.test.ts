...
describe('R076 luck-cycle pattern break/recovery cases', () => {
  it('preserves break, rescue, blocking, completion, and change cases separately', () => {
    ...
    expect(R076_CASES.filter((x) => x.provenanceKind === 'PARAPHRASED_SOURCE_CASE')).toHaveLength(3);
    expect(R076_CASES.filter((x) => x.provenanceKind === 'DIRECT_QUOTE')).toHaveLength(2);
