...
import {
  R020_AUTHORITY,
  R020_MINGLI_TANYUAN_TOPOLOGY,
  R020_PRIMITIVE_COMPARISON,
  R020_TEXTUAL_OVERLAP,
  R020_TRADITION_SURFACES,
...
describe('R020 cross-school strength primitives', () => {
  it('tracks primary commentary surfaces separately from compilation title witnesses', () => {
    expect(R020_CROSS_SCHOOL_STRENGTH_VERSION).toBe('0.4.0-research');
    expect(R020_TRADITION_SURFACES.map((surface) => surface.id)).toEqual([
      'XU_ZIPING_PINGZHU',
      'REN_DITIAN_SUI_CHANWEI',
      'MINGLI_TANYUAN_COMPILATION_WITNESS',
    ]);
  ...
  it('preserves dependency uncertainty while closing the bounded comparison', () => {
  ...
  it('forbids fake cross-school confidence and numeric/final classification', () => {
    expect(R020_PRIMITIVE_COMPARISON.universalNumericStrengthScoreSupported).toBe(false);
    ...
    expect(R020_AUTHORITY.generalizedStrengthClassifierAuthorized).toBe(false);
    expect(R020_AUTHORITY.productionAuthorityPromoted).toBe(false);
  });
});
