...
function annualRequest(year: number): ReadingRequest {
  return {
    requestId: `business-annual-${year}`,
...
function temporalFacts(value: CanonicalSajuSnapshot, year: number) {
  ...
  if (context === undefined) throw new Error('business annual request must produce temporal context');
...
describe('MyeongHa Business Annual T9 reading candidate', () => {
  it('reuses the existing shared-channel-gated Business Natal T8 registry and remains research-only', () => {
    ...
    expect(BUSINESS_ANNUAL_READING_METHODOLOGY.status).toBe('research');
    expect(BUSINESS_ANNUAL_READING_METHODOLOGY.family).toBe('time_dynamics');
  ...
  it('emits exactly one personalized Business Annual activation from request-scoped temporal facts', () => {
    ...
    expect(activations[0]?.taxonomy).toEqual({ tier: 'T9', category: 'business', subcategory: 'annual' });
  ...
  it('makes business/annual complete only with Business Natal T8 plus Business Annual T9 evidence', () => {
    const natal = snapshot();
    ...
    const composition = buildReadingCompositionEvidence(natal, execution, registry, request, {
      narrativePolicyVersion: 'myeongha-business-annual-narrative-v1',
    });

    expect(composition.selection.coverageState).toBe('complete');
    expect(composition.selection.missingRequirements).toEqual([]);
    expect(composition.selection.profileRef?.id).toBe('myeonghwa-reading-profile-business-annual-v1');
  ...
  it('is deterministic for the same target year and changes annual business semantics for another year', () => {
    const natal = snapshot();
    const registry = createBusinessAnnualReadingCandidateRegistry();
    ...
    expect(
      nextYear.claims.find((claim) => claim.claimType === BUSINESS_ANNUAL_THEME_CLAIM_TYPE)?.value,
    ).not.toEqual(
      first.claims.find((claim) => claim.claimType === BUSINESS_ANNUAL_THEME_CLAIM_TYPE)?.value,
    );
  });

  it('bounds annual branch clash to business-operating adjustment pressure', () => {
    const natal = withDayBranchClash(snapshot());
    const execution = runInterpretation(natal, createBusinessAnnualReadingCandidateRegistry(), {
      temporalFacts: temporalFacts(natal, 2026),
      now: NOW,
    });
    const tension = execution.claims.find(
      (claim) =>
        claim.state === 'active' &&
        claim.claimType === BUSINESS_ANNUAL_TENSION_CLAIM_TYPE &&
