...
describe('R079 multi-cycle conflict-resolution corpus', () => {
  ...
  it('keeps direct propositions contextual and non-executable', () => {
    ...
    expect(R079_DIRECT_PROPOSITIONS.filter((x) => x.provenanceKind === 'DIRECT_QUOTE')).toHaveLength(2);
    expect(R079_DIRECT_PROPOSITIONS.filter((x) => x.provenanceKind === 'BOUNDED_QUOTED_EXCERPT')).toHaveLength(1);
    expect(R079_DIRECT_PROPOSITIONS.filter((x) => x.provenanceKind === 'PARAPHRASED_SOURCE_CLASS')).toHaveLength(1);
