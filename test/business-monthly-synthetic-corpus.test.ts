...
import { createBusinessMonthlyReadingCandidateRegistry } from '../src/research/business-monthly-reading-candidate.js';
...
const FIXED_CALCULATION_TIME = new Date('2026-09-05T00:40:00+09:00');
const FIXED_INTERPRETATION_TIME = new Date('2026-09-05T00:41:00+09:00');
const NARRATIVE_POLICY_VERSION = 'myeongha-business-monthly-narrative-v1';
const NARRATIVE_POLICY_KEY = `myeongha-business-monthly@${NARRATIVE_POLICY_VERSION}`;
...
function consumedInputFingerprint(
  ...
  return `business_monthly_consumed_input_${deterministicContentHash({
    domain: 'business',
...
function interpretationSignature(
  ...
  if (selectedClaims.length === 0) throw new Error('Expected selected Business Monthly claims.');
  ...
  return `business_monthly_interpretation_${deterministicContentHash({
    domain: 'business',
    temporalScope: 'monthly',
    targetMonth: TARGET_MONTH,
    claims: materials,
...
function buildObservation(caseId: string, month: number, day: number, hour: number): BenchmarkAObservation {
  const registry = createBusinessMonthlyReadingCandidateRegistry(FIXED_CALCULATION_TIME.toISOString());
  ...
  if (temporalContext === undefined || temporalContext.scope !== 'monthly') {
    throw new Error(`Business Monthly temporal context missing for ${caseId}.`);
  }
  ...
  if (composition.evidence === undefined || composition.selection.coverageState !== 'complete') {
    throw new Error(`Business Monthly complete evidence missing for ${caseId}.`);
  }
  ...
  if (
    ...
    !selectedClaims.some((claim) => claim.taxonomy.tier === 'T9' && claim.taxonomy.subcategory === 'monthly')
  ) {
    throw new Error(`Business Monthly T8/T9 composition missing for ${caseId}.`);
  }

  const fallback = buildValidatedDeterministicFallback(composition.evidence.bundle, NARRATIVE_PROFILES);
  if (!fallback.validation.valid) throw new Error(`Invalid Business Monthly fallback for ${caseId}.`);
...
describe('MyeongHa Business Monthly synthetic corpus', () => {
  it('keeps all identity signatures deterministic', () => {
    const first = buildObservation('business-monthly-determinism', 1, 9, 9);
    const repeated = buildObservation('business-monthly-determinism', 1, 9, 9);
  ...
  it(
    'measures 500 actual Business Monthly readings and rejects clones or false diversity',
    () => {
      ...
      outer: for (let month = 1; month <= 12; month += 1) {
        for (let day = 1; day <= 28; day += 1) {
          for (const hour of SYNTHETIC_HOURS) {
            ...
            const caseId = `business-monthly-synthetic-${String(scannedBirthInputs).padStart(4, '0')}`;
      ...
      process.stdout.write(
        `BUSINESS_MONTHLY_SYNTHETIC_MEASUREMENT ${JSON.stringify({
