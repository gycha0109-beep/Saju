...
import { createWealthAnnualReadingCandidateRegistry } from '../src/research/wealth-annual-reading-candidate.js';
...
const NARRATIVE_POLICY_VERSION = 'myeongha-wealth-annual-narrative-v1';
const NARRATIVE_POLICY_KEY = `myeongha-wealth-annual@${NARRATIVE_POLICY_VERSION}`;
...
function wealthAnnualConsumedInputFingerprint(
  ...
  return `wealth_annual_consumed_input_${deterministicContentHash({
    domain: 'wealth',
...
function wealthAnnualInterpretationSignature(
  execution: ReturnType<typeof runInterpretation>,
  selectedClaimIds: readonly string[],
): string {
  const selectedClaims = selectedWealthClaims(execution.claims, selectedClaimIds);
  if (selectedClaims.length === 0) throw new Error('Expected selected Wealth Annual claims.');
  ...
  return `wealth_annual_interpretation_${deterministicContentHash({
    domain: 'wealth',
    temporalScope: 'annual',
    claims: materials,
  })}`;
...
function buildObservation(
  ...
  const registry = createWealthAnnualReadingCandidateRegistry(FIXED_CALCULATION_TIME.toISOString());
  ...
  if (temporalContext === undefined || temporalContext.scope !== 'annual') {
    throw new Error(`Wealth Annual temporal context missing for ${caseId}.`);
  }
  ...
  if (composition.evidence === undefined) {
    throw new Error(`Wealth Annual evidence bundle missing for ${caseId}.`);
  }
  if (composition.selection.coverageState !== 'complete') {
    throw new Error(
      `Wealth Annual coverage was ${composition.selection.coverageState} for ${caseId}.`,
    );
  }
  ...
  if (
    ...
    throw new Error(`Wealth Annual T8/T9 composition missing for ${caseId}.`);
  }
  ...
  if (!fallback.validation.valid) {
    throw new Error(`Invalid Wealth Annual fallback for ${caseId}.`);
  }
  ...
  return {
    ...
    consumedInputFingerprint: wealthAnnualConsumedInputFingerprint(
      execution,
      composition.selection.selectedClaimIds,
    ),
    interpretationSignature: wealthAnnualInterpretationSignature(
      execution,
      composition.selection.selectedClaimIds,
    ),
...
describe('MyeongHa Wealth Annual synthetic corpus', () => {
  it('keeps calculation, consumed-input, interpretation, and narrative signatures deterministic', () => {
    const first = buildObservation('wealth-annual-determinism', 1, 9, 9);
    const repeated = buildObservation('wealth-annual-determinism', 1, 9, 9);
  ...
  it(
    'measures 500 actual Wealth Annual readings and rejects cross-semantic clones or false diversity',
    () => {
      ...
      outer: for (let month = 1; month <= 12; month += 1) {
        for (let day = 1; day <= 28; day += 1) {
          for (const hour of SYNTHETIC_HOURS) {
            ...
            const caseId = `wealth-annual-synthetic-${String(scannedBirthInputs).padStart(4, '0')}`;
      ...
      process.stdout.write(
        `WEALTH_ANNUAL_SYNTHETIC_MEASUREMENT ${JSON.stringify({
