...
describe('R063 Day self/spouse domain boundary', () => {
  ...
  it('preserves bounded Day spouse-configuration examples without generalizing them', () => {
    expect(R063_CONFIGURATION_EXAMPLES).toHaveLength(2);
    expect(R063_CONFIGURATION_EXAMPLES).toEqual(expect.arrayContaining([
      expect.objectContaining({
        sourceSurface: '日坐命財更在財生旺之鄉主得妻財又主妻賢明',
        daySeatSpecific: true,
        generalizedSpouseOutcomeAuthorized: false,
        executable: false,
      }),
      expect.objectContaining({
        sourceSurface: '或日為妻如在空刑克煞之地言克妻妾之斷',
        generalizedSpouseOutcomeAuthorized: false,
        executable: false,
      }),
    ]));
