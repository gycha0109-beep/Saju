...
describe('R081 source-bounded classical worked-example corpus', () => {
  ...
  it('never treats source examples as independent history or engine ground truth', () => {
    expect(R081_EXAMPLES.filter((x) => x.sourceConclusionProvenance === 'DIRECT_QUOTE_EXCERPT')).toHaveLength(3);
    expect(R081_EXAMPLES.filter((x) => x.sourceConclusionProvenance === 'NORMALIZED_SOURCE_CONCLUSION')).toHaveLength(1);
    expect(R081_EXAMPLES.filter((x) => x.sourceConclusionProvenance === 'BOUNDED_SOURCE_PARAPHRASE')).toHaveLength(2);
