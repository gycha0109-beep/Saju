...
function forceContext(report: ReturnType<typeof buildI26ChallengeContextAvailabilityV10>, mechanism: string) {
...
function buildAligned(pillars: StructuralPillarInput) {
  ...
  const report = buildI26ChallengeContextAvailabilityV10(
    input.review,
    input.force,
    input.roots,
    input.relations,
...
describe('I26 v10 challenge context availability with I42 stem scope closure', () => {
  test('closes day-stem scope transfer and transformed-target adoption as live stem blockers', () => {
    const { report } = buildAligned(stemCombinationPillars());
    const output = forceContext(report, 'OUTPUT_LEAKAGE');

    expect(report.stemTransformationScopeClosureAccepted).toBe(true);
    ...
    expect(output?.existingCapabilities.some((item) => item.startsWith('I42 stem transformation scope closure:'))).toBe(true);
  });

  test('redirects the former stem transformation graph-evaluation route to binding/interaction rather than no-effect', () => {
    const { report } = buildAligned(stemCombinationPillars());
    const output = forceContext(report, 'OUTPUT_LEAKAGE');
  ...
  test('does not alter branch three-combination policy dependencies', () => {
  ...
  test('fails closed when I42 does not preserve the negative-scope contract', () => {
    const input = buildInputs(stemCombinationPillars());
    const invalidScope = {
      ...input.stemScope,
      reviewId: `${input.stemScope.reviewId}_invalid`,
      dayStemHuaQiResultContractDirectTransferAuthorized: true,
    } as unknown as typeof input.stemScope;
    const report = buildI26ChallengeContextAvailabilityV10(
      input.review,
      input.force,
      input.roots,
    ...
    expect(report.stemTransformationScopeClosureAccepted).toBe(false);
    expect(output?.unresolvedCapabilities).toContain(
      'challenge-target stem-combination day-stem reference scope-transfer policy',
    );
    expect(output?.unresolvedCapabilities).toContain(
  ...
  test('keeps deterministic partial/no-effect/no-scoring guards after negative scope closure', () => {
    const aligned = buildAligned(stemCombinationPillars());
    const repeated = buildAligned(stemCombinationPillars());
