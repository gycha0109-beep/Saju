  ...
  it('preserves selected-source wording and Day-target scope', () => {
    ...
    expect(R078_SELECTED_SOURCE_DEFINITIONS).toEqual([
      expect.objectContaining({
        term: 'FANYIN',
        sourceRepresentation: '若歲運與日相對，謂之返吟',
        provenanceKind: 'DIRECT_QUOTE',
        temporalActors: ['SUIYUN', 'DAY'],
      }),
      expect.objectContaining({
        term: 'FUYIN',
        sourceRepresentation: '歲運壓日，謂之伏吟',
        provenanceKind: 'DIRECT_QUOTE',
        temporalActors: ['SUIYUN', 'DAY'],
      }),
    ]);
    expect(R078_SELECTED_SOURCE_DEFINITIONS.every((x) => x.provenanceKind === 'DIRECT_QUOTE')).toBe(true);
