...
import { createCareerMonthlyReadingCandidateRegistry } from '../src/research/career-monthly-reading-candidate.js';
...
const NARRATIVE_POLICY_VERSION = 'myeongha-career-monthly-narrative-v1';
const NARRATIVE_POLICY_KEY = `myeongha-career-monthly@${NARRATIVE_POLICY_VERSION}`;
...
function careerMonthlyConsumedInputFingerprint(
  ...
  return `career_monthly_consumed_input_${deterministicContentHash({
    domain: 'career',
...
function careerMonthlyInterpretationSignature(
  execution: ReturnType<typeof runInterpretation>,
  selectedClaimIds: readonly string[],
): string {
  const selectedClaims = selectedCareerClaims(execution.claims, selectedClaimIds);
  if (selectedClaims.length === 0) throw new Error('Expected selected Career Monthly claims.');
  ...
  return `career_monthly_interpretation_${deterministicContentHash({
    domain: 'career',
    temporalScope: 'monthly',
    claims: materials,
  })}`;
...
function buildObservation(
  ...
  const registry = createCareerMonthlyReadingCandidateRegistry(FIXED_CALCULATION_TIME.toISOString());
  ...
  if (temporalContext === undefined || temporalContext.scope !== 'monthly') {
    throw new Error(`Career Monthly temporal context missing for ${caseId}.`);
  }
  ...
  if (composition.evidence === undefined) {
    throw new Error(`Career Monthly evidence bundle missing for ${caseId}.`);
  }
  if (composition.selection.coverageState !== 'complete') {
    throw new Error(`Career Monthly coverage was ${composition.selection.coverageState} for ${caseId}.`);
  }
  ...
  if (
    ...
    throw new Error(`Career Monthly T8/T9 composition missing for ${caseId}.`);
  }
  ...
  if (!fallback.validation.valid) {
    throw new Error(`Invalid Career Monthly fallback for ${caseId}.`);
  }
  ...
  return {
    ...
    consumedInputFingerprint: careerMonthlyConsumedInputFingerprint(
      execution,
      composition.selection.selectedClaimIds,
    ),
    interpretationSignature: careerMonthlyInterpretationSignature(
      execution,
      composition.selection.selectedClaimIds,
    ),
...
describe('MyeongHa Career Monthly synthetic corpus', () => {
  it('keeps calculation, consumed-input, interpretation, and narrative signatures deterministic', () => {
    const first = buildObservation('career-monthly-determinism', 1, 9, 9);
    const repeated = buildObservation('career-monthly-determinism', 1, 9, 9);
  ...
  it(
    'measures 500 actual Career Monthly readings and rejects cross-semantic clones or false diversity',
    () => {
      ...
      outer: for (let month = 1; month <= 12; month += 1) {
        for (let day = 1; day <= 28; day += 1) {
          for (const hour of SYNTHETIC_HOURS) {
            ...
            const caseId = `career-monthly-synthetic-${String(scannedBirthInputs).padStart(4, '0')}`;
      ...
      process.stdout.write(
        `CAREER_MONTHLY_SYNTHETIC_MEASUREMENT ${JSON.stringify({
