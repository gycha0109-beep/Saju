...
describe('R074 annual stem/branch precedence traditions', () => {
  it('preserves stem emphasis without discarding the branch', () => {
    ...
    expect(R074_PROPOSITIONS.filter((x) => x.provenanceKind === 'DIRECT_QUOTE')).toHaveLength(3);
    expect(R074_PROPOSITIONS.filter((x) => x.provenanceKind === 'DIRECT_QUOTE_EXCERPTS')).toHaveLength(1);
