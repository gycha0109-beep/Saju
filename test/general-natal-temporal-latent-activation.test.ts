...
describe('R073 temporal activation vs natal latent interaction', () => {
  it('preserves latent, activation, temporary, and context states separately', () => {
    ...
    expect(R073_STATE_MODEL.filter((x) => x.provenanceKind === 'DIRECT_QUOTE')).toHaveLength(3);
    expect(R073_STATE_MODEL.filter((x) => x.provenanceKind === 'PARAPHRASED_SOURCE_CLASS')).toHaveLength(1);
    expect(R073_STATE_MODEL.filter((x) => x.provenanceKind === 'BOUNDED_QUOTED_EXCERPT')).toHaveLength(1);
