...
describe('R077 luck-cycle Yong/Xi support vs shift boundary', () => {
  it('separates natal roles, luck interaction, natal rescue, and structural change', () => {
    ...
    expect(R077_ROLE_STATES.filter((x) => x.provenanceKind === 'DIRECT_QUOTE_EXCERPT')).toHaveLength(1);
    expect(R077_ROLE_STATES.filter((x) => x.provenanceKind === 'DIRECT_QUOTE_EXCERPTS')).toHaveLength(1);
    expect(R077_ROLE_STATES.filter((x) => x.provenanceKind === 'DIRECT_QUOTE')).toHaveLength(1);
    expect(R077_ROLE_STATES.filter((x) => x.provenanceKind === 'BOUNDED_RESEARCH_ABSTRACTION')).toHaveLength(1);
