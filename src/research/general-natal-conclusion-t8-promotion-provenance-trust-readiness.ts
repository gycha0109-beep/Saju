...
export const GENERAL_NATAL_CONCLUSION_T8_PROMOTION_PROVENANCE_TRUST_READINESS_VERSION =
  'myeonghwa-general-natal-conclusion-t8-promotion-provenance-trust-readiness-v2' as const;
...
export const GENERAL_NATAL_CONCLUSION_T8_PROMOTION_CONTROL_IDS = Object.freeze([
  ...
  'PASSAGE_WITNESS_SUBSET_IS_NOT_EXACT_CONSUMER_SUPPORT',
  'SINGLE_SOURCE_PEER_WITNESS_IS_NOT_MULTI_SOURCE_PROVENANCE',
  'SOURCE_INTEGRITY_QUALIFICATION_REMAINS_REQUIRED',
  'MODERN_CONSUMER_SEMANTIC_BRIDGE_REMAINS_REQUIRED',
...
function planContainsRuleIds(planRuleIds: ReadonlySet<string>, ruleIds: readonly string[]): boolean {
...
export function buildGeneralNatalConclusionT8PromotionProvenanceTrustReadiness() {
  ...
  const exactConsumerPassageSupportReady =
    passageEvidence.counts.exactCurrentRuleProductionSupportedCount ===
    passageEvidence.counts.exactCandidateRuleCount;
  const productionEligibleProvenanceByRegistryContract =
    sourceTierPermitted && productionRuleProvenanceReady;
  const productionEligibleProvenanceEstablished =
    productionEligibleProvenanceByRegistryContract && exactConsumerPassageSupportReady;
  ...
  const material = {
    ...
    issue: '#730' as const,
    auditBaseSha: '3b724b087311a9831cc695466b1fff9861ad57aa' as const,
    ...
    runtimeSourceAuthority: {
      ...
      sourceReferenceRegistered,
      sourceTierPermitted,
      productionEligibleProvenanceByRegistryContract,
      productionEligibleProvenanceEstablished,
    },
    runtimePassageAuthority: {
      evidenceId: passageEvidence.evidenceId,
      passageWitnessSubsetEstablished:
        passageEvidence.authorityBoundary.immutablePassageWitnessSubsetEstablished,
      exactCandidateRuleCount: passageEvidence.counts.exactCandidateRuleCount,
      passagePinnedRuleCount: passageEvidence.counts.passagePinnedRuleCount,
      singleSourcePassagePinnedRuleCount:
        passageEvidence.counts.singleSourcePassagePinnedRuleCount,
      multiSourcePassagePinnedRuleCount:
        passageEvidence.counts.multiSourcePassagePinnedRuleCount,
      unsupportedExactRuleCount: passageEvidence.counts.unsupportedExactRuleCount,
      exactCurrentRuleProductionSupportedCount:
        passageEvidence.counts.exactCurrentRuleProductionSupportedCount,
      peerFamilyPassageWitnessEstablished:
        passageEvidence.authorityBoundary.peerFamilyPassageWitnessEstablished,
      peerFamilyMultiSourceWitnessEstablished:
        passageEvidence.authorityBoundary.peerFamilyMultiSourceWitnessEstablished,
      sourceIntegrityQualificationStillRequired:
        passageEvidence.authorityBoundary.sourceIntegrityQualificationStillRequired,
      modernConsumerSemanticBridgeStillRequired:
        passageEvidence.authorityBoundary.modernConsumerSemanticBridgeStillRequired,
      exactConsumerPassageSupportReady,
    },
    ...
    productionContract: {
      ...
      passageWitnessDoesNotAutomaticallyPromoteRuleQuality: true as const,
    ...
    guardrails: {
      passageWitnessTreatedAsAutomaticPromotion: false as const,
    ...
    recommendedNextAction:
      'QUALIFY_PASSAGE_WITNESSES_AND_ESTABLISH_EXACT_CONSUMER_SEMANTIC_SUPPORT_BEFORE_REVIEW_AND_LIFECYCLE_PROMOTION' as const,
  };
