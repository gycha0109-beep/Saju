...
export const R126_ORDERING_CASES: readonly R126OrderingCase[] = Object.freeze([
  ...
  },,
  {
    ...
    actors: ['FOOD_GOD', 'OWL_OR_RESOURCE'],
    chain: ['食神', '逢梟', '食受制'],
    sourceStatement: 'The source preserves 食神逢梟 as a bounded failure relation.',
    interpretiveReading:
      'Output can itself be constrained or damaged by a resource-like actor, so output presence alone does not settle its effective role.',
    researchInference:
      'Output effectiveness is relational and cannot be modeled as a fixed leakage amount.',
    relationPresenceSettlesOutcome: false,
    genericChallengingCollapseAuthorized: false,
    scalarAggregationAuthorized: false,
    independentCountingAuthorized: false,
    ...
    finalStrengthAuthorized: false,
    causalChainDoubleCountingRisk: true,
    unresolvedFactors: ['梟/印 exact resolver remains outside R126'],
    prohibitedExtensions: [
      'OUTPUT_PRESENCE_EQUALS_EFFECTIVE_LEAKAGE',
      'RESOURCE_ALWAYS_SUPPORTIVE',
    ],
  },
  {
    caseId: 'R126-C22-WEALTH-PROTECTS-FOOD',
    comparisonGroupId: 'OUTPUT-LEAKAGE-CONTEXT',
    family: 'OUTPUT',
    scope: 'PATTERN_CONTEXT',
    orderingType: 'RESCUE_CHAIN',
    orderingEvidence: 'EXPLICIT_IN_SOURCE',
    sourceNature: 'BASE_TEXT',
    sourceStratum: '子平真詮 / 食神救應',
    sourceRefs: [
      'src/research/general-natal-food-god-pattern-conditions.ts',
      'https://www.ncc.com.tw/fate/paleo/bg/bg_033.htm',
    ],
    actors: ['FOOD_GOD', 'OWL_OR_RESOURCE', 'WEALTH'],
    chain: ['食逢梟', '生財', '護食'],
    sourceStatement: 'The source preserves 生財以護食 as a rescue path after 食 meets 梟.',
    interpretiveReading:
      'Wealth can protect output inside a local rescue chain.',
    researchInference:
      'Output and wealth cannot be assigned independent fixed negative polarity without considering their local relation.',
    relationPresenceSettlesOutcome: false,
    genericChallengingCollapseAuthorized: false,
    scalarAggregationAuthorized: false,
    independentCountingAuthorized: false,
    ...
    finalStrengthAuthorized: false,
    causalChainDoubleCountingRisk: true,
    unresolvedFactors: ['the rescue path is pattern-local'],
    prohibitedExtensions: [
      'WEALTH_ALWAYS_DRAINS_OUTPUT_CONTEXT',
      'OUTPUT_AND_WEALTH_ALWAYS_STACK_ADVERSELY',
    ],
  },
  {
    caseId: 'R126-C23-WEAK-BODY-HEAVY-OFFICER-RESOURCE-MEDIATION',
    comparisonGroupId: 'CONTROL-BODY-CONTEXT',
    family: 'CONTROL',
    scope: 'BOTH_EXPLICIT',
    orderingType: 'RESCUE_CHAIN',
    orderingEvidence: 'EXPLICIT_IN_SOURCE',
    sourceNature: 'LATER_COMMENTARY',
    sourceStratum: '子平真詮評註 / 官格成敗',
    sourceRefs: ['https://www.ncc.com.tw/fate/paleo/bg/bg_033.htm'],
    actors: ['DAY_MASTER', 'OFFICER', 'RESOURCE'],
    chain: ['身弱官重', '印化官'],
    sourceStatement:
      'The commentary preserves a weak-body heavy-officer context in which resource mediates/transforms the officer relation.',
    interpretiveReading:
      'Control pressure and its settlement depend on body condition and a mediating resource path.',
    researchInference:
      'Officer presence cannot be converted into a fixed negative unit independent of body and mediation context.',
    relationPresenceSettlesOutcome: false,
    genericChallengingCollapseAuthorized: false,
    scalarAggregationAuthorized: false,
    independentCountingAuthorized: false,
    ...
    finalStrengthAuthorized: false,
    causalChainDoubleCountingRisk: true,
    unresolvedFactors: ['化官 is preserved semantically, not as an executable numeric transform'],
    prohibitedExtensions: [
      'OFFICER_EQUALS_FIXED_NEGATIVE_WEIGHT',
      'RESOURCE_MEDIATION_EQUALS_NUMERIC_CANCELLATION',
    ],
  },
  {
    caseId: 'R126-C24-LIGHT-WEALTH-HEAVY-PEER',
    comparisonGroupId: 'WEALTH-CAPACITY-CONTEXT',
    family: 'WEALTH_CAPACITY',
    scope: 'PATTERN_CONTEXT',
    orderingType: 'CONTEXT_DEPENDENT',
    ...
    actors: ['WEALTH', 'PEER'],
    chain: ['財輕', '比重', '財被分奪'],
    sourceStatement: 'The source preserves 財輕比重 as a bounded failure condition.',
    interpretiveReading:
      'Wealth outcome depends on relative configuration with peer support rather than wealth presence alone.',
    researchInference:
      'Wealth is not a context-free drain/expenditure signal and relative light/heavy language supplies no numeric scalar.',
    relationPresenceSettlesOutcome: false,
    genericChallengingCollapseAuthorized: false,
    scalarAggregationAuthorized: false,
    independentCountingAuthorized: false,
    ...
    finalStrengthAuthorized: false,
    causalChainDoubleCountingRisk: false,
    unresolvedFactors: ['財輕 and 比重 lack executable thresholds'],
    prohibitedExtensions: [
      'WEALTH_PRESENCE_EQUALS_FIXED_DRAIN',
      'CAI_QING_BI_ZHONG_EQUALS_NUMERIC_RATIO',
    ],
  }
]);
