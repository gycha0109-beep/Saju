...
export const GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_VERSION = '0.1.0-research' as const;
export const GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_SCOPE =
  'jia_day_master_gui_visible_stem_to_zhengyin_exact_relation_observation' as const;
export const GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_DECISION =
  'AUTHORIZED_RESEARCH_ONLY' as const;

export const GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_SOURCE = Object.freeze({
  title: '子平真詮 / 子平真詮評註',
...
export const GENERAL_NATAL_JIA_GUI_ZHENGYIN_SOURCE_CONTEXT_TEXT =
  '偏財為母之正夫者，譬如甲以癸為正印，戊為偏財，戊癸合也；丙以乙為正印，庚為偏財，乙庚合也。餘可類推。' as const;
export const GENERAL_NATAL_JIA_GUI_ZHENGYIN_SOURCE_CLAUSE = '甲以癸為正印' as const;

export interface JiaGuiZhengyinExactPairInput {
  readonly dayMaster: HeavenlyStem;
  readonly visibleCounterpartStem: HeavenlyStem;
}

export type JiaGuiZhengyinExactRelationState =
  | 'jia_gui_zhengyin_relation_observed'
  | 'outside_selected_source_pair_scope';

export interface JiaGuiZhengyinExactRelationEvaluation {
  readonly state: JiaGuiZhengyinExactRelationState;
  readonly dayMaster: HeavenlyStem;
  readonly visibleCounterpartStem: HeavenlyStem;
  readonly sourceRelation: '正印' | null;
  readonly sourceClause: typeof GENERAL_NATAL_JIA_GUI_ZHENGYIN_SOURCE_CLAUSE | null;
  readonly exactRelationObserved: boolean;
  readonly authority: 'research_only';
}

export function observeJiaGuiZhengyinExactRelation(
  input: JiaGuiZhengyinExactPairInput,
): JiaGuiZhengyinExactRelationEvaluation {
  if (input.dayMaster === '갑' && input.visibleCounterpartStem === '계') {
    return Object.freeze({
      state: 'jia_gui_zhengyin_relation_observed',
      dayMaster: input.dayMaster,
      visibleCounterpartStem: input.visibleCounterpartStem,
      sourceRelation: '正印',
      sourceClause: GENERAL_NATAL_JIA_GUI_ZHENGYIN_SOURCE_CLAUSE,
...
export const GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_UNAUTHORIZED_DERIVATIONS =
  Object.freeze([
...
export const GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_DEFINITION_HASH = createHash('sha256')
  .update(
    JSON.stringify({
      version: GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_VERSION,
      scope: GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_SCOPE,
      decision: GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_DECISION,
      source: GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_SOURCE,
      sourceContextText: GENERAL_NATAL_JIA_GUI_ZHENGYIN_SOURCE_CONTEXT_TEXT,
      sourceClause: GENERAL_NATAL_JIA_GUI_ZHENGYIN_SOURCE_CLAUSE,
      exactDayMaster: '갑',
      exactVisibleCounterpartStem: '계',
      sourceRelation: '正印',
...
export const GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_AUTHORITY = Object.freeze({
  version: GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_VERSION,
  definitionHash: GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_DEFINITION_HASH,
  decision: GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_DECISION,
  upstreamYinshouSourceCategoryVersion:
    GENERAL_NATAL_YINSHOU_ZHENG_PIAN_SOURCE_CATEGORY_VERSION,
  upstreamYinshouSourceCategoryDefinitionHash:
    GENERAL_NATAL_YINSHOU_ZHENG_PIAN_SOURCE_CATEGORY_DEFINITION_HASH,
  directSourceJiaGuiZhengyinObserved: true,
  canonicalDayMasterPathGoverned,
  canonicalVisibleStemPathGoverned,
  exactJiaGuiPairRepresentable: canonicalDayMasterPathGoverned && canonicalVisibleStemPathGoverned,
  jiaGuiZhengyinExactPairMatcherAuthorizedResearchOnly: true,
  ...
  unauthorizedDerivations:
    GENERAL_NATAL_JIA_GUI_ZHENGYIN_EXACT_RELATION_UNAUTHORIZED_DERIVATIONS,
  authorityBoundary:
    'The selected source directly states the exact relation 甲以癸為正印. Current canonical contracts expose governed day-master and visible-pillar stem identities, so the literal 甲/癸 pair is exactly representable as a bounded research-only relation observation. This authority does not equate canonical 정인 with source 正印, generalize 正印 to other stem pairs including the separately mentioned 丙/乙 example, operationalize 餘可類推, recompute or consume canonical Ten-God facts, scan a chart, consume hidden stems or branch Ten-Gods, resolve 正印 into an 印綬 runtime constituent, establish 黨眾/助寡 or final 強弱/旺衰, derive Gyeokguk, or emit production facts.',
});
