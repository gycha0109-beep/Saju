...
import type { CareerTenGodChannel } from './career-natal-reading-schema.js';

export interface CareerNatalSemanticText {
  headline: string;
  summary: string;
}
...
const COPY: Readonly<Record<TenGod, CareerNarrativeCopySpec>> = Object.freeze({
  비견: {
...
export function careerNatalSemanticText(
  god: TenGod,
  channel: CareerTenGodChannel,
): CareerNatalSemanticText {
  const copy = COPY[god];
  return {
    headline: `${CHANNEL_HEADLINE_CONTEXT[channel]} — ${copy.headline}`,
    summary: channel === 'visible_stems' ? copy.visibleSummary : copy.branchSummary,
  };
}
