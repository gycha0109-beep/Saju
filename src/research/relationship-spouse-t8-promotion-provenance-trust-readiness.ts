...
export function buildRelationshipSpouseT8PromotionProvenanceTrustReadiness() {
  ...
  const ruleSourceIds = Object.freeze(
    [...new Set(registry.rules.flatMap((rule) => rule.sourceRefs.map((sourceRef) => sourceRef.sourceId)))].sort(),
  );
  const registeredSourceIds = Object.freeze(registry.sources.map((source) => source.sourceId).sort());
  const sourceTiers = Object.freeze(registry.sources.map((source) => source.provenanceTier).sort());
  ...
  // The admitted Spouse T8 bundle and #681 readiness review bind no ReviewerTrustContext.
  // Generic trust helpers and test fixtures are infrastructure, not concrete authority.
  ...
  const material = {
    ...
    issue: '#696' as const,
    auditBaseSha: 'c146bc45011fd84ed7f5f003f73de47fe94bdbc1' as const,
