# FR269 — Controlled Capture Geometry Sensitivity

Watchtower-Track: face-reading

## 1. Purpose

FR269 freezes the completed controlled FR257 evidence set and turns it into a deterministic descriptive sensitivity report.
...
FR269 does not issue a capture threshold, pass/fail rule, calibration, correction formula, interpretation claim, Production authority, or Commerce authority.
...
The committed FR269 report records:
...
The low-angle condition also changes screen face-box scale materially relative to baseline. Therefore FR269 does not claim that vertical orientation alone causally explains the metric movement.

The observed response is not treated as linear.

## 4. Analyzer

`analyzeControlledCaptureGeometrySensitivityFR269` validates the three FR257 bundles and emits:
...
`scripts/analyze-fr269-controlled-geometry.mjs --verify` deterministically regenerates the report from committed evidence and verifies it against the committed report JSON.
...
FR269 therefore adds an ephemeral observer hook to FR257. During a future live shutter event only, FR269 may consume the transient screen landmarks and FR76 canonical metric landmarks and persist scalar diagnostic values only:
...
FR76 already inverse-pose-aligns runtime metric landmarks into canonical metric geometry before the frozen eye metric is computed. FR269 therefore does **not** invent an additional pose-normalization correction. Its explicit state is:

`not_issued_fr76_metric_geometry_is_already_inverse_pose_aligned`

## 6. Privacy boundary

FR269 diagnostic persistence is scalar-only.
...
- face embeddings;
- identity templates;
- participant/operator/provider identifiers in the final FR269 sidecar.

## 7. Next decision

After a small controlled set of new FR269 diagnostic captures, compare whether the low-angle failure is already visible in screen-space eye tilt, appears only after FR76 metric reconstruction, or remains in both.
...
No threshold or correction formula is authorized by FR269.
