...
import { buildDeterministicStillImageDiagnosticReportFR274 } from '/face/observable-morphology-deterministic-still-image-diagnostic-fr274.js';
...
async function buildRuntimeInputs() {
  ...
  const fr66 = await runPhotoToLipsContourNeutralSurfaceFR66({
    ...
    providerRunRef: 'fr274:parity-bootstrap',
...
function downloadJson(value) {
  ...
  anchor.href = url;
  anchor.download = 'myeongha-fr274-still-image-diagnostic-'
...
async function analyze() {
  ...
  finishedReport = null;
  setStatus(elements.analysisStatus, '6장 분석 중…');
  ...
  const geometryCollector = createCaptureGeometryAttributionCollectorFR257({
    onEphemeralGeometry: diagnosticCollector.observe,
  });
  const observations = [];
  ...
  try {
    for (let index = 0; index < SLOTS.length; index += 1) {
      ...
      const providerRunRef = 'fr274:still:' + String(index + 1);
      let prepared;
      try {
        ...
        if (geometryEvidence === null || diagnosticEvidence === null) {
          throw new Error(slot.label + ' did not produce complete FR257/FR269 evidence.');
        }
        if (
          geometryEvidence.primaryMetric.value
          !== diagnosticEvidence.fr76CanonicalMetricEyeOuterCornerTiltMeanDegrees
        ) {
          throw new Error(slot.label + ' frozen metric drifted from FR269 FR76 diagnostic.');
        }
        ...
        observations.push(Object.freeze({
          ...
          fr76CanonicalMetricEyeOuterCornerTiltMeanDegrees:
            diagnosticEvidence.fr76CanonicalMetricEyeOuterCornerTiltMeanDegrees,
          screenMinusFr76Degrees: diagnosticEvidence.screenMinusFr76Degrees,
          ...
          relativeRotationFromFirstFrontRadians:
            geometryEvidence.geometry.relativeRotationFromFirstAcceptedCaptureRadians,
          inPlaneLateralAxisOrientationRadians:
            geometryEvidence.geometry.inPlaneLateralAxisOrientationRadians,
          poseUniformScaleComponent: geometryEvidence.geometry.poseUniformScaleComponent,
          screenFaceBoxWidthFraction: geometryEvidence.geometry.screenFaceBoxWidthFraction,
          screenFaceBoxHeightFraction: geometryEvidence.geometry.screenFaceBoxHeightFraction,
    ...
    finishedReport = buildDeterministicStillImageDiagnosticReportFR274({
      generatedAt: new Date().toISOString(),
      observations,
    });
    elements.resultJson.textContent = JSON.stringify(finishedReport, null, 2);
    elements.resultCard.hidden = false;
    setStatus(elements.analysisStatus, '완료 · scalar-only report 6/6');
  } finally {
    updateAnalyzeState();
  }
}
